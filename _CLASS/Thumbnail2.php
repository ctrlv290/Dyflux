...
Class Thumbnail2 extends Dbconn
{
	// {{{ Variables
	 ...
	 */
	public function create($file_idx, $save_filename, $width = null, $height = null, $scale = 'exactfit', $export = 'jpg', $force_create = false)
	{
		$export = (strtolower($export) != "png" && strtolower($export) != "jpg") ? "jpg" : strtolower($export);

		//File Table 확인
		$chk = $this -> chkFile($file_idx, $save_filename);
		...
		//썸네일 생성 여부 확인
		$isExists = false;
		$original_fullpath = DY_PATH . '/' . $chk["save_path"] . '/' . $chk["save_filename"];
		$original_pathinfo = pathinfo($original_fullpath);
		$thumbnail_filename = $original_pathinfo["filename"] . '_thumb_' . $width . '_' . $height . '.' . $export;

		$savepath = DY_THUMBNAIL_PATH . '/' . $thumbnail_filename;
		//echo $savepath;

		$isExists = file_exists(DY_THUMBNAIL_PATH . '/' . $thumbnail_filename);
		if(!$isExists || $force_create) {
			$filepath = $original_fullpath;

			// 원본 이미지가 없는 경우
			if (!file_exists($filepath))
				$this->raiseError('#Error: Thumbnail::create() : File not found or permission error.' . ' at ' . __LINE__);

			// 섬네일 크기가 잘못 지정된 경우
			if ($width <= 1 && $height <= 1)
				$this->raiseError('#Error: Thumbnail::create() : Invalid thumbnail size.' . ' at ' . __LINE__);
			...
			if (!in_array($export, Array('jpg', 'gif', 'png')))
				$this->raiseError('#Error: Thumbnail::create() : Invalid export format.' . ' at ' . __LINE__);
			...
			if (($type[2] < 1 || $type[2] > 3) ||
				...
				$this->raiseError('#Error: Thumbnail::create() : Filetype not supported. Thumbnail not created.' . ' at ' . __LINE__);
			}
			...
			$thumbfilename = basename($savepath);
			...
			if ($iserror)
				$this->raiseError('#Error: Thumbnail::create() : invalid path or permission error.'.' at '. __LINE__);
		...
		return array(
			...
			"src" => '/' . DY_DATA_DIR . '/' . DY_THUMBNAIL_DIR . '/' . $thumbfilename,
			"url" => DY_THUMBNAIL_URL . '/' . $thumbfilename
		);
	}// END: function create();
	...
	public function chkFile($file_idx, $save_filename)
	{
		$qry = "

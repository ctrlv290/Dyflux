...
class CS extends DBConn
{
	 ...
	 * @param string $order_cs_status
	 * @return array
	 */
	public function getOrderDetailRelateOrderPackIdx($order_pack_idx, $sort_by = "O.order_idx", $sort_type = "asc", $order_cs_status = "", $return_add = "")
	{
        $qry_where = "";
		if($order_cs_status != "") {
            $qry_where = "And M.order_cs_status = N'$order_cs_status'";
		}
        $join_stock_return = "";
        $qry_where = "";
        if($return_add == "Y"){
            $join_stock_return = "Left Outer Join (
				                                    SELECT order_matching_idx
				                                    From DY_STOCK
				                                    WHERE stock_is_del = N'N' And stock_kind = 'BACK'
				                    ) As ST On ST.order_matching_idx = M.order_matching_idx
				                    ";
            $qry_where .= "AND ST.order_matching_idx IS NULL";
        }
		$qry = "
			Select
			O.*, P.product_name, P.product_sale_type, PO.product_option_name
		     ...
		     , Case When P.product_sale_type = 'SELF' Then CONVERT(IFNULL(S.stock_amount_NORMAL, 0), char(100))
		    ...
		    , M.order_cs_status
		    , (Select code_name From DY_CODE DCC Where DCC.parent_code = N'ORDER_MATCHING_CS' And DCC.code = M.order_cs_status) as order_cs_status_han
			, ROW_NUMBER() OVER(PARTITION BY O.order_idx Order by M.order_matching_idx ASC) as inner_no
			From DY_ORDER O
				Left Outer Join DY_ORDER_PRODUCT_MATCHING M On O.order_idx = M.order_idx And M.order_matching_is_del = N'N' 
	...
	public function getOrderPriorityList($order_idx, $product_option_idx)
	{
		$qry = "
			Select 
			    ...
			    , IFNULL(DATE_FORMAT(O.order_progress_step_accept_date, '%Y-%m-%d'), '') as order_progress_step_accept_date
				, S.seller_name
				, O.receive_name
				, IFNULL(DATE_FORMAT(O.invoice_priority_date, '%Y-%m-%d'), '') as  invoice_priority_date
				, IFNULL(MEM.member_id, '') as member_id
			From DY_ORDER O
			Inner Join DY_ORDER_PRODUCT_MATCHING M On O.order_idx = M.order_idx
	...
	public function getStockCount($product_option_idx)
	{
		$qry = "
				...
				, IFNULL(S.stock_amount_NORMAL, 0) as stock_amount_NORMAL
				From 
					DY_PRODUCT P
	...
	public function setOrderHold($order_pack_idx, $hold_yn, $cs_msg, $set_alarm = "N", $set_alarm_datetime = "")
	{
		...
		$qry = "
			Update DY_ORDER
				Set order_is_hold = '$hold_yn', order_is_hold_date = NOW()
					, order_moddate = NOW(), order_modip = N'$modip', last_member_idx = N'$last_member_idx'
	...
	public function updateOrderCancelAll($order_pack_idx, $cs_reason_code1, $cs_reason_code2, $cs_msg)
	{
		...
		foreach($_opm_list as $ord){
			...
			// 가접수 상태는 정산과 상관없음
			if($_pack_order_progress_step != "ORDER_ACCEPT_TEMP") {
				//정산테이블 입력
	...
	public function restoreOrderByPackage($order_pack_idx, $cs_msg) {
		return $this->restoreOrder($order_pack_idx, "order_pack_idx", $cs_msg);
	}

	public function restoreOrderByMatching($order_matching_idx, $cs_msg) {
		return $this->restoreOrder($order_matching_idx, "order_matching_idx", $cs_msg);
	}
	 ...
	 * 접수, 배송 주문만 정상복귀 가능
	 * 송장 상태의 주문은 정상복귀 불가
	 * 200423 수정 kyu - 복귀는 취소로 인한 정산 데이터를 삭제한다
	 * @param $target_idx
	 * @param $target_type
	 * @param $cs_msg
	 * @return bool
	 */
	private function restoreOrder($target_idx, $target_type, $cs_msg){
		global $GL_Member;
		$mod_ip = $_SERVER["REMOTE_ADDR"];
		$last_member_idx = $GL_Member["member_idx"];

		if($target_type != "order_pack_idx" && $target_type != "order_matching_idx") return false;

		// 송장 상태 인지 체크
		$qry = "
			Select COUNT(O.order_idx) 
			From DY_ORDER AS O JOIN DY_ORDER_PRODUCT_MATCHING AS OPM ON O.order_idx = OPM.order_idx
			Where $target_type = N'$target_idx' AND order_progress_step = 'ORDER_INVOICE'
		";

		parent::db_connect();
		$ops_invoice_cnt = parent::execSqlOneCol($qry);
		parent::db_close();

		if($ops_invoice_cnt > 0){
			return false;
		}
		...
		$qry_where = "";
		if($target_type == "order_pack_idx") {
			$qry_where = " AND O.order_pack_idx = N'$target_idx' ";
		} elseif($target_type == "order_matching_idx") {
			$qry_where = " AND OPM.order_matching_idx = N'$target_idx'";
		}
		...
		//취소된 주문 만
		$qry = "
			Select O.order_idx, O.order_pack_idx, O.order_progress_step
		...
		$opm_list = parent::execSqlList($qry);

		foreach($opm_list as $ord) {
			$order_idx = $ord["order_idx"];
			...
			$qry = "
				...
				WHERE
					settle_is_del = N'N'
					AND order_cs_status = 'ORDER_CANCEL'
					AND $target_type = N'$target_idx'
			";
			parent::execSqlUpdate($qry);

			//CS 입력
			$cs_task = $target_type == "order_pack_idx" ? "ORDER_RESTORE_ALL" : "ORDER_RESTORE_ONE";
			$cs_rst = $this->insertCS($order_idx, $order_pack_idx, $order_matching_idx, $product_idx, $product_option_idx, 'Y', '', $cs_task, $cs_msg, '', '', null, false, "", "");
			if(!$cs_rst) $return_value = false;
		}

		if($return_value) {
			parent::sqlTransactionCommit();
		} else {
			parent::sqlTransactionRollback();
		}
				...
				foreach($product_list as $prod){
					...
					if($is_auto_stock_order == "Y"){
						...
						if($_product_sale_type == "SELF") {
							//배송처리된 재고 찾기
							$qry = "
								Select stock_idx, stock_is_parent_confirm_date, stock_amount
								From DY_STOCK S 
								Where 
								      stock_is_del = N'N'
								...
								Order by stock_is_parent_confirm_date DESC

							";

							$_stock_idx = parent::execSqlList($qry);
                            if ($_stock_idx) {
                                foreach ($_stock_idx as $stock) {
                                    if ($product_option_cnt == 0) {
                                        break;
                                    }
                                    //현 금액의 재고 수량
                                    $_currentStockPriceAmount = intval($stock["stock_amount"]);

                                    if ($product_option_cnt > $_currentStockPriceAmount) {
                                        //현재 금액의 재고가 부족일 경우
                                        //현재 금액의 재고만큼만 차감 후 Next
                                        $product_option_cnt = $product_option_cnt - $_currentStockPriceAmount;
                                    } else {
                                        $_currentStockPriceAmount = $product_option_cnt;
                                        $product_option_cnt = 0;
                                    }
                                    $stock_is_parent_confirm_date = $stock["stock_is_parent_confirm_date"];
                                    $stock_idx = $stock["stock_idx"];
                                    //입고예정 등록
                                    $qry = "
									Insert Into DY_STOCK
									(
									 stock_ref_idx, product_idx, product_option_idx, order_matching_idx, stock_is_parent_confirm_date
									 ...
									 SELECT 
										0, product_idx, product_option_idx, N'$order_matching_idx' ,N'$stock_is_parent_confirm_date'
										, N'BACK', order_idx, 0, N'N', N''
										, N'', N''
										, N'0', N'$delivery_return_type'
										, stock_unit_price, N'$_currentStockPriceAmount', N'0', N''
        ...
        if($product_list){
            ...
            $_product_sale_type = parent::execSqlOneCol($qry);
                if($_product_sale_type == "SELF") {
                    //배송처리된 재고 찾기
                    $qry = "
                        Select stock_idx , stock_is_parent_confirm_date, stock_unit_price, stock_amount
                        From DY_STOCK S 
                        Where 
                              stock_is_del = N'N'
                              And order_matching_idx = N'$order_matching_idx'
                    ...
                    $return_product_list = parent::execSqlList($qry);

                    //이전 회수요청 된 재고 찾기
                    $qry = "
                        Select stock_idx , stock_is_parent_confirm_date, stock_unit_price, stock_due_amount
                        From DY_STOCK S 
                        Where 
                              stock_is_del = N'N'
                              And order_matching_idx = N'$order_matching_idx'
                              ...
                              And product_option_idx = N'$product_option_idx'
                              And order_idx = N'$order_idx'
                              And stock_kind = N'BACK'
                        GROUP BY stock_unit_price , stock_is_parent_confirm_date
                        Order by stock_is_parent_confirm_date DESC;
                        ";

                    $return_stock_list = parent::execSqlList($qry);

                    //이전 회수 요청 건 차감
                    if($return_stock_list) {

                        foreach ($return_product_list as $in_key => $in) {
                            $key = array_search($in['stock_unit_price'], array_column($return_stock_list, 'stock_unit_price'));
                            if ($in['stock_amount'] < $return_stock_list[$key]['stock_due_amount']) {
                                $return_stock_list[$key]["stock_due_amount"] -= $in['stock_amount'];
                                $return_product_list[$in_key]['stock_amount'] = 0;
                            } else if ($in['stock_amount'] >= $return_stock_list[$key]['stock_due_amount']) {
                                $return_product_list[$in_key]['stock_amount'] -= $return_stock_list[$key]["stock_due_amount"];
                                $return_stock_list[$key]["stock_due_amount"] = 0;
                            }
                        }
                    }
                    ...
                    if ($return_product_list) {
                        foreach ($return_product_list as $stock) {
	...
	public function changeOrderProduct($order_pack_idx, $order_idx, $order_matching_idx, $c_product_idx, $c_product_option_idx, $c_product_option_cnt, $c_product_sale_price, $c_add_price, $cs_reason_code1, $cs_reason_code2, $cs_msg)
	{
		...
		$opm_idx = $_opm["order_matching_idx"];
		...
		if($_pack_order_progress_step == "ORDER_SHIPPED")
		{
			...
			$qry = "
				...
				Select
					N'$_new_order_idx', N'$_new_order_idx', seller_idx, N'ORDER_ACCEPT', NOW(), NOW(), N'MANUAL',
			        order_pack_code, order_pay_date, CONCAT('C', market_order_no) AS market_order_no, market_order_no_is_auto, market_order_subno, market_product_no, market_product_no_is_auto,
			        market_product_name, market_product_option, market_order_id, order_unit_price, order_amt, order_pay_amt, order_calculation_amt, order_cnt, delivery_fee,
				    order_pay_type, order_name, order_tp_num, order_hp_num, order_addr1, order_addr2, order_zipcode, 
			        receive_name, receive_tp_num, receive_hp_num, receive_addr1, receive_addr2, receive_zipcode, receive_memo,
			...
			$qry = "
				...
				(
				    ...
				    N'$product_option_sale_price', N'$c_product_option_purchase_price', 
				    N'N', NOW(), N'$modip', N'$last_member_idx'
				)
			";
			...
			if($c_product_sale_type == "CONSIGNMENT") {
				$settle_con = new Settle();
				$settle_con->insertSettleFromOrder($_new_order_idx, "order_idx", $c_product_sale_type);
			}


		}else{
			//배송 전 교환
			//200428 배송전 상품 교환은 매입매출에 -, + 로 추가되지 않도록
			...
			$qry = "
				Update DY_ORDER_PRODUCT_MATCHING
					Set 
						...
						, product_option_purchase_price = N'$c_product_option_purchase_price'
			...
			//교환 전 상품 위탁 상품이라면, 기존 정산 삭제, 가접수 제외
			if($_pack_order_progress_step == "ORDER_ACCEPT" && $product_sale_type == "CONSIGNMENT") {
				if($c_product_sale_type == "CONSIGNMENT") {
					$settle_con->updateSettleFromOrder($order_idx, "order_idx", $c_product_sale_type, $order_matching_idx, "EXCHANGE");
				} else {
					$settle_con->deleteSettle($order_idx, "order_idx", "order_matching_idx = N'$opm_idx'");
				}
			} else {
				if($c_product_sale_type == "CONSIGNMENT") {
					$settle_con->insertSettleFromOrder($order_idx, "order_idx", $c_product_sale_type, $order_matching_idx, "EXCHANGE");
				}
			}
		}
				...
				if($_prod) {
					...
					if($inserted){
						...
						if ($_pack_order_progress_step == "ORDER_ACCEPT") {
							$settle_con = new Settle();
							$settle_con->insertSettleFromOrder($order_pack_idx, "order_pack_idx", "CONSIGNMENT", $inserted);
						}
	...
	public function copyOrderOne($order_idx, $copy_seller_idx, $copy_product_idx, $copy_product_option_idx, $copy_product_option_cnt, $copy_product_option_sale_price, $cs_msg)
	{
		...
		$qry = "
			...
			Select
				...
				CONCAT(market_order_no, '_', (Select count(*) + 1 From DY_ORDER Where market_order_no like CONCAT(O.market_order_no, '_%'))),
		...
		if ($originOrderProgressStep == "ORDER_ACCEPT") {
			$settle_con = new Settle();
			$settle_con->insertSettleFromOrder($_new_order_idx, "order_idx", "CONSIGNMENT", $_m_idx);
		}
	...
	public function copyOrderWhole($order_idx, $copy_seller_idx, $cs_msg)
	{
		...
		$qry = "
			...
			Select
				...
				N'$originOrderProgressStep', order_progress_step_accept_temp_date, order_progress_step_accept_date, order_progress_step_accept_member_idx, N'MANUAL',
				order_pack_code, order_pay_date
				, market_order_no + '_' + CONVERT((Select count(*) + 1 From DY_ORDER Where market_order_no like O.market_order_no + '_%'), CHAR(100))
				, market_order_no_is_auto, market_order_subno, market_product_no, market_product_no_is_auto,
				market_product_name, market_product_option, market_order_id, order_unit_price, order_amt, order_pay_amt, order_calculation_amt, order_cnt, delivery_fee,
				order_pay_type, order_name, order_tp_num, order_hp_num, order_addr1, order_addr2, order_zipcode,
				receive_name, receive_tp_num, receive_hp_num, receive_addr1, receive_addr2, receive_zipcode, receive_memo,
		...
		if ($originOrderProgressStep == "ORDER_ACCEPT") {
			$settle_con = new Settle();
			$settle_con->insertSettleFromOrder($_new_order_idx, "order_idx", "CONSIGNMENT");
		}
	...
	public function getOrderReturnList($order_idx){
		...
		$qry = "
			Select 
		       R.*
			, (Select delivery_name From DY_DELIVERY_CODE D Where D.delivery_code = R.delivery_code LIMIT 1) as delivery_name
	...
	public function getCSList($order_pack_idx, $cs_task = "", $search_column = "", $search_keyword = "", $order_by = "CS.cs_idx desc")
	{
		...
		$qry = "
		    ...
		    , IFNULL(C2.code_name, '') as cs_reason_text
			, M.member_id, U.name
			, Case When CS.cs_file_idx1 <> 0 THEN (Select CONCAT(user_filename, '|' , save_filename) From DY_FILES Where file_idx = CS.cs_file_idx1) Else '' End as filename1
			, Case When CS.cs_file_idx2 <> 0 THEN (Select CONCAT(user_filename, '|' , save_filename) From DY_FILES Where file_idx = CS.cs_file_idx2) Else '' End as filename2
			, Case When CS.cs_file_idx3 <> 0 THEN (Select CONCAT(user_filename, '|' , save_filename) From DY_FILES Where file_idx = CS.cs_file_idx3) Else '' End as filename3
			, Case When CS.cs_file_idx4 <> 0 THEN (Select CONCAT(user_filename, '|' , save_filename) From DY_FILES Where file_idx = CS.cs_file_idx4) Else '' End as filename4
			, Case When CS.cs_file_idx5 <> 0 THEN (Select CONCAT(user_filename, '|' , save_filename) From DY_FILES Where file_idx = CS.cs_file_idx5) Else '' End as filename5
			From DY_ORDER_CS CS
			...
			Left Outer Join DY_MEMBER_USER U On U.member_idx = M.idx
			Where
				CS.cs_is_del = N'N'
				And 
			      (
	...
	public function setCSConfirm($cs_idx){
		...
		$qry = "
			Update DY_ORDER_CS
			Set cs_confirm = N'Y', cs_confirm_date = NOW()
			, cs_moddate = NOW(), cs_modip = N'$modip', last_member_idx = N'$last_member_idx'
	...
	public function getMyAlarmList(){
		...
		$qry = "
			...
			Where 
				...
				And A.cs_alarm_datetime = DATE_FORMAT('$search_time_start', '%Y-%m-%d %H:%i:%s');
		";
	...
	public function clearMyAlarm($cs_alarm_idx){
		...
		$qry = "
			...
			Set cs_alarm_confirm = N'Y'
				, cs_alarm_moddate = NOW(), cs_alarm_modip = N'$modip'
				, last_member_idx = N'$last_member_idx'
	 ...
	 */
	public function insertSettleCancel($order_matching_idx, $cs_reason_code2 = "", $requireDbConnection = false, $ShippedCancel = false){
		global $GL_Member;
		$modip = $_SERVER["REMOTE_ADDR"];
		$last_member_idx = $GL_Member["member_idx"];
		...
		if($_order_list){
			...
			foreach ($_order_list as $ord){
				...
				if($seller_type == "MARKET_SELLER" || $seller_type == "CUSTOM_SELLER"){
					$settle_conn = new Settle();
					$_market_commission = $settle_conn->getSettleCommission($seller_idx, $product_idx, $product_option_idx, $order_unit_price);
	 ...
	 * @return array
	 */
	public function getCSReturnStatistics($search_start_date,$search_end_date){
		$_date_ary = array();
//		$query_start_date = date('Y-m-d', strtotime('-14 days'));
//		$query_end_date = date('Y-m-d', time());
        $search_start_date = date($search_start_date, time());
        $search_end_date = date($search_end_date, time());
		$start_date = strtotime($search_start_date);
		$end_date = strtotime($search_end_date);
        ...
        $qry = "CALL str_split('".implode(",", $_date_ary)."',',');";
        $this->multiQuery($qry);
		...
		$qry = "
            Select
                val as date
                , IFNULL(Sum(Case When cancel_reason = 'RETURN_REFUND' Then 1 Else 0 End), 0) as  RETURN_REFUND
                , IFNULL(Sum(Case When cancel_reason = 'RETURN_POOR' Then 1 Else 0 End), 0) as  RETURN_POOR
                , IFNULL(Sum(Case When cancel_reason = 'RETURN_DELIVERY_ERR' Then 1 Else 0 End), 0) as  RETURN_DELIVERY_ERR
                , IFNULL(Sum(Case When cancel_reason = 'CANCEL_LOSS' Then 1 Else 0 End), 0) as  CANCEL_LOSS
                , IFNULL(Sum(Case When cancel_reason = 'CANCEL_SOLDOUT' Then 1 Else 0 End), 0) as  CANCEL_SOLDOUT
                , IFNULL(Sum(Case When cancel_reason = 'CANCEL_DELIVERY_DELAY' Then 1 Else 0 End), 0) as  CANCEL_DELIVERY_DELAY
                 
                , IFNULL(Sum(Case When change_reason = 'EXCHANGE_NORMAL' Then 1 Else 0 End), 0) as  EXCHANGE_NORMAL
                , IFNULL(Sum(Case When change_reason = 'EXCHANGE_POOR' Then 1 Else 0 End), 0) as  EXCHANGE_POOR
                , IFNULL(Sum(Case When change_reason = 'EXCHANGE_DELIVERY_ERR' Then 1 Else 0 End), 0) as  EXCHANGE_DELIVERY_ERR
                , IFNULL(Sum(Case When change_reason = 'EXCHANGE_SOLDOUT' Then 1 Else 0 End), 0) as  EXCHANGE_SOLDOUT
                , IFNULL(Sum(Case When change_reason = 'EXCHANGE_PRODUCT_CHANGE' Then 1 Else 0 End), 0) as  EXCHANGE_PRODUCT_CHANGE
            From
            (
            ...
            Left Outer Join 
            (
                Select cs_reason_code2 as cancel_reason, DATE_FORMAT(product_cancel_date, '%Y-%m-%d') as cancel_date
                From DY_ORDER_PRODUCT_MATCHING
                Where order_matching_is_del = N'N' And order_cs_status = N'ORDER_CANCEL'
                    And product_cancel_date >= '$search_start_date 00:00:00'
                    And product_cancel_date <= '$search_end_date 23:59:59'
            ) cc On cc.cancel_date = DateTable.val
            Left Outer Join 
            (
                Select 
                    cs_reason_code2 as change_reason, DATE_FORMAT(product_change_date, '%Y-%m-%d') as change_date
                From DY_ORDER_PRODUCT_MATCHING
                Where order_matching_is_del = N'N' And order_cs_status = N'PRODUCT_CHANGE'
                    And product_change_date >= '$search_start_date 00:00:00'
                    And product_change_date <= '$search_end_date 23:59:59'
            ) ch On ch.change_date = DateTable.val
            
            Group by DateTable.val
            Order by DateTable.val ASC
		";
	...
	public function updateSellerCancelConfirm($order_idx, $confirm_val){
		...
		$qry = "
			...
			, order_seller_cancel_confirm_date = NOW()
			, order_seller_cancel_confirm_member_idx = N'$last_member_idx'
			Where order_idx = N'$order_idx'
		";
	...
	public function updateSellerCancelOffConfirm($order_idx, $confirm_val){
		...
		$qry = "
			...
			, order_seller_cancel_off_confirm_date = NOW()
	...
	public function getLatestShippingInfo(){
		$qry = "
			Select receive_name, receive_tp_num, receive_hp_num, receive_addr1, receive_addr2, receive_zipcode, receive_memo 
			From DY_ORDER 
			Where 

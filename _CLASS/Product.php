...
class Product extends Dbconn
{
	...
	public function gotoTrashProduct($product_idx)
	{
		...
		$qry = "
			Update DY_PRODUCT
			Set product_is_trash = N'Y', product_is_trash_date = getdate()
			, product_moddate = getdate(), product_modip = N'$product_modip'
	...
	public function restoreTrashProduct($product_idx_list)
	{
		...
		$qry = "
			Update DY_PRODUCT
			Set product_is_trash = N'N', product_is_trash_date = getdate()
			, product_moddate = getdate(), product_modip = N'$product_modip'
			, last_member_idx = N'$last_member_idx'
					...
					if($product_detail_idx[$i] == "") {
						...
						$qry = "
							...
							Set 
								...
								product_detail_moddate = getdate(),
	...
	public function getProductDataByDelete($product_idx)
	{
		$qry = "
			Select P.*
		        , (Select manage_group_idx From DY_MANAGE_GROUP MG Where MG.manage_group_idx = SP.manage_group_idx) as supplier_group_idx
		        , (Select manage_group_idx From DY_MANAGE_GROUP MG Where MG.manage_group_idx = S.manage_group_idx) as seller_group_idx
				, isNull((Select Sum(stock_amount * stock_type) From DY_STOCK T Where T.stock_is_del = N'N' And T.stock_is_confirm = N'Y' And T.stock_status = N'NORMAL' And T.product_idx = P.product_idx), 0) as current_stock_count
	...
	public function updateProductOption($args){
		...
		$qry = "
			...
			Set product_option_name = N'$product_option_name'
				...
				, product_option_moddate = getdate()
		...
		if(in_array($soldout_type, $able_field) && in_array($change_value, array("Y", "N")))
		{
			$qry = "
				...
				Set
					...
					product_option_soldout_date = getdate(),
	...
	public function getProductOptionDataByDelete($product_options_idx)
	{
		$qry = "
			Select P.*
		        , (Select manage_group_idx From DY_MANAGE_GROUP MG Where MG.manage_group_idx = SP.manage_group_idx) as supplier_group_idx
		        , (Select manage_group_idx From DY_MANAGE_GROUP MG Where MG.manage_group_idx = S.manage_group_idx) as seller_group_idx
				, isNull((Select Sum(stock_amount * stock_type) From DY_STOCK T Where T.stock_is_del = N'N' And T.stock_is_confirm = N'Y' And T.stock_status = N'NORMAL' And T.product_option_idx = PO.product_option_idx), 0) as current_stock_count
	...
	public function updateProductXlsSelected($target, $target_idx, $target_field, $val)
	{
		...
		if($target == "product") {
			...
			$require_update_field = ", product_moddate = getdate(), product_modip = N'$product_modip', last_member_idx = N'$last_member_idx'";
		}elseif($target == "product_option"){
			$table_name = "DY_PRODUCT_OPTION";
			$table_key_field = "product_option_idx";
			$require_update_field = ", product_option_moddate = getdate(), product_option_modip = N'$product_modip', last_member_idx = N'$last_member_idx'";
		}else{
			return false;
		}
	...
	public function dupCheckProductCommissionMarketProductNo($seller_idx, $market_product_no)
	{
		$return = false;
		$qry = "
			Select count(*) From DY_MARKET_COMMISSION Where seller_idx = N'$seller_idx' And market_product_no = N'$market_product_no' And comm_is_del = N'N'
		";
		parent::db_connect();
	...
	public function insertProductCommission($seller_idx, $market_product_no, $market_commission, $delivery_commission, $product_idx_list, $product_option_idx_list)
	{
		...
		$qry = "
			...
			(seller_idx, market_product_no, market_commission, delivery_commission, comm_regip, last_member_idx)
			VALUES 
			(
			 N'$seller_idx'
			 , N'$market_product_no'
	 ...
	 * @return bool
	 */
	public function updateProductCommission($comm_idx, $market_commission, $delivery_commission, $product_idx_list, $product_option_idx_list)
	{
		global $GL_Member;
		$modip = $_SERVER["REMOTE_ADDR"];
		$last_member_idx = $GL_Member["member_idx"];
	...
	function getSoldOutList() {
		$qry = "
			SELECT
				...
				ISNULL(product_option_soldout_date, N'') AS product_option_soldout_date,
				ISNULL(product_option_soldout_memo, N'') AS product_option_soldout_memo

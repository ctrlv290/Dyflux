...
//$idx = $_GET["idx"];
$supplier_idx = $_GET["supplier_idx"];            //파일 생성 로그 IDX
$order_download_file_idx = $_GET["order_download_file_idx"];            //파일 생성 로그 IDX
...
if($order_download_file_idx)
{
	...
	$_file_view = $C_Order -> getOrderDownloadFileLog($order_download_file_idx);
	$order_download_filename = $_file_view["order_download_file_name"];
}else{
...
$down_filename_prefix = "주문다운로드";
if($_supplier_view){
	$down_filename_prefix = $_supplier_view["supplier_name"] . "_" . $down_filename_prefix;
}


if($_file_view) {

	$filepath      = DY_ORDER_DOWNLOAD_PATH . "/" . $order_download_filename;
	$user_filename = $down_filename_prefix."_".$order_download_filename;
	...
	if (file_exists($filepath)) {
		...
		if (isset($_GET["order_download_email_idx"])) {
			$C_Order->insertOrderDownloadDocumentDownLog($supplier_idx, $order_download_file_idx, $_GET["order_download_email_idx"]);
		}

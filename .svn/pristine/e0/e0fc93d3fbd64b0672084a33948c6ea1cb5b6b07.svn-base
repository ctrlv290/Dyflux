...
class Bank extends DBConn
{
	 ...
	 * @return int
	 */
	public function insertBankAccount($bank_type, $bank_name, $bank_sort, $bank_is_use, $bank_start_date, $bank_use_n_date)
	{
		global $GL_Member;
		...
		$qry = "
			...
			(bank_type, bank_name, bank_sort, bank_is_use, bank_start_date, bank_use_n_date, bank_regip, bank_regidx) 
			VALUES
			(
			 N'$bank_type'
			 , N'$bank_name'
	 ...
	 */
	public function updateBankAccount($bank_idx, $bank_type, $bank_name, $bank_is_use, $bank_use_n_date)
	{
		global $GL_Member;
		$modip = $_SERVER["REMOTE_ADDR"];
		$last_member_idx = $GL_Member["member_idx"];
		...
		$qry = "
			...
			Set
				...
				, bank_use_n_date = N'$bank_use_n_date'
				, bank_modip = N'$modip'
				, bank_modidx = N'$last_member_idx'
				, bank_moddate = NOW()
	 ...
	 * @return array
	 */
	public function getBankTransactionCheck($bank_use_n_date, $today, $bank_idx)
	{
		$qry = "
				Select
				BANK.bank_name
				...
				From DY_BANK_ACCOUNT BANK
				Left Outer Join DY_BANK_TRANSACTION T On BANK.bank_idx = T.bank_idx
				Where BANK.bank_is_del = N'N'
				AND T.tran_is_del = N'N' 
				AND T.tran_date >= N'$bank_use_n_date' 
				AND T.tran_date <= N'$today'
				AND BANK.bank_idx = N'$bank_idx'
				GROUP BY T.bank_idx
		";
	...
	public function getTodayBankTransaction($date)
	{
		$qry = "
			...
			From DY_BANK_ACCOUNT B
				Left Outer Join DY_BANK_TRANSACTION T On T.tran_is_del = N'N' And T.tran_date = convert(N'$date', DATE) And T.bank_idx = B.bank_idx
				Left Outer Join DY_CODE C On C.parent_code = N'BANK_TYPE' And B.bank_type = C.code
			Where B.bank_is_del = N'N'
			AND B.bank_start_date <= N'$date'
			AND (B.bank_use_n_date > N'$date' OR B.bank_use_n_date = N'0000-00-00')
	...
	public function saveTodayBankTransaction($date, $bank_idx, $tran_in, $tran_out, $tran_memo) {
		...
		$tranIdx = $this->execSqlOneCol("select tran_idx From DY_BANK_TRANSACTION Where tran_is_del = N'N' And bank_idx = N'$bank_idx' And tran_date = N'$date'");
		if (!$tranIdx) $tranIdx = NULL;

		$qry = "
			Insert Into DY_BANK_TRANSACTION
			(tran_idx, tran_date, bank_idx, tran_in, tran_out, tran_sum, tran_memo, tran_regip, tran_regidx)
			VALUES
			(
				N'$tranIdx'
				, N'$date'
	...
	public function getTodayBankTransactionDetail($period_type, $date, $bank_type)
	{
		...
		if($period_type == "day") {
			$qry = "
				...
				AND Bank.bank_is_del = N'N'
				AND Bank.bank_start_date <= N'$date'
				AND (BANK.bank_use_n_date > N'$date' OR BANK.bank_use_n_date = N'0000-00-00')
			...
			$qry = "
				...
				AND BANK.bank_is_del = N'N'
				AND Bank.bank_start_date <= N'$date'
				AND (BANK.bank_use_n_date > N'$prev_date' OR BANK.bank_use_n_date = N'0000-00-00')
				Order By BANK.bank_sort ASC
			";

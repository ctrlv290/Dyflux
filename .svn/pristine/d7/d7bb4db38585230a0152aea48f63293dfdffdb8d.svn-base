...
$order_by = "A.file_down_regdate DESC";
...
if($_search_param)
{
	...
	if($_search_paramAryList["date_start"] != "" && $_search_paramAryList["date_end"] != ""){
		$qryWhereAry[] = "	 
			A.file_down_regdate >= '".$_search_paramAryList["date_start"]." 00:00:00' 
			And A.file_down_regdate <= '".$_search_paramAryList["date_end"]." 23:59:59' 
		";
	}
...
// make select query
$args['qry_table_idx'] 	= "A.file_down_idx";
...
$args['qry_table_name'] 	= " 
								DY_LEDGER_FILE_DOWN_LOG A
								Left Outer Join DY_LEDGER_EMAIL E On A.email_idx = E.email_idx
								Left Outer Join DY_LEDGER_FILE F On A.file_idx = F.file_idx
";
if($ledger_type == "LEDGER_PURCHASE"){
	$args['qry_table_name'] 	.= " 
...
$args['qry_where']			= " 1=1 And F.ledger_type = N'$ledger_type' ";

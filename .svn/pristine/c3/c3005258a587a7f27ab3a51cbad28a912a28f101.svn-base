...
class Stock extends DBConn
{
		...
		$qry = "
			Select 
				O.*
			     ...
			     , CONCAT(S.supplier_addr1, ' ', S.supplier_addr2) as supplier_info_addr 
	...
	/**
	 * 상품 옵션 정보 Detail 반환 + 현재고수량(정상재고, 원가별 정상재고)
	 * @param $product_options_idx
	 * @return array|false|null
	 */
	public function getProductOptionDetailWithStockAmount($product_options_idx, $stock_unit_price){
		...
		$qry = "
			...
			, (
			  ...
			  (
			  SELECT 
		          Sum(Case When stock_status = 'NORMAL' And stock_unit_price = '$stock_unit_price' Then stock_amount * stock_type Else 0 END)
			  From DY_STOCK ST
			  Where ST.product_option_idx = O.product_option_idx
			        And ST.stock_is_del = N'N'
			        And ST.stock_is_confirm = N'Y'
			  ) as stock_amount_unit_price_NORMAL

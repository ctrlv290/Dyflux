...
// make select query
$args['qry_table_idx'] 	= "B.bank_idx";
$args['qry_get_colum'] 	= " B.* , C.code_name as bank_type_han, M.member_id
                            ";

$args['qry_table_name'] 	= " DY_BANK_ACCOUNT B
							...
							";
$args['qry_where']			= " bank_is_del = N'N' ";
if(count($qryWhereAry) > 0)
{
	$args['qry_where'] .= " And " . join(" And ", $qryWhereAry);
}
$args['qry_groupby']		= "";
$args['qry_orderby']		= "B.bank_idx ASC ";
...
$xls_header = array(
	...
	array(
		"header_name" => "사용여부",
		"field_name" => "bank_is_use",
		"width" => 18,
	),
    array(
        "header_name" => "사용시작일",
        "field_name" => "bank_start_date",
        "width" => 18,
    ),
    array(
        "header_name" => "사용중지일",
        "field_name" => "bank_use_n_date",
        "width" => 18,
    ),
	array(
		"header_name" => "작업자",
...
foreach($_list as $row_num => $row) {
	...
	foreach ($xls_row as $cellValue) {
		...
		if($field_name == "bank_use_n_date" && $xls_row[2] == "Y"){
            $cellValue = "";
		}

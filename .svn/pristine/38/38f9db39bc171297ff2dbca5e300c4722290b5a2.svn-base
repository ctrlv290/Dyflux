...
var Common = {
	...
	newWinPopup : function(url, popup_id, wW, wH, isScroll) {
		...
		let childWindow = window.open(url, popup_id, "width=" + wW + ",height=" + wH + ",top=" + pos_y + ",left=" + pos_x + ",status=yes,resizable=0,scrollbars=" + isScroll);
		window[popup_id] = childWindow;
	...
	addCommas: function(val){
		if(typeof val != "undefined" && val != null && val != "undefined") {
			...
			let signedStr = valStr.split('.');
			if (signedStr.length > 2 || signedStr.length == 0) {
				return val;
			}

			let str = signedStr[0].replace(/\B(?=(\d{3})+(?!\d))/g, ",");
			if (signedStr.length == 2) {
				str = str + '.' + signedStr[1];
			}
	...
	jqGridRefresh : function (selector, page, param) {

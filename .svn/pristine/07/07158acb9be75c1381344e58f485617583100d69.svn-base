...
<!--						</div>-->
						<div class="finder_col">
							<span class="text">배송상태</span>
							<select name="order_progress_step">
								<option value="">전체</option>
								<option value="ORDER_ACCEPT" <?=($order_progress_step == "ORDER_ACCEPT") ? "selected" : ""?>>접수</option>
								<option value="ORDER_SHIPPED" <?=($order_progress_step == "ORDER_SHIPPED") ? "selected" : ""?>>배송</option>
							</select>
						</div>
						<div class="finder_col">
							<span class="text">확인여부</span>
							<select name="order_seller_cancel_confirm">
								<option value="">전체</option>
		...
		<div class="grid_btn_set_top">
			<a href="javascript:;" class="btn btn-cancel-confirm-all">전체확인</a>
			<a href="javascript:;" class="btn btn-cancel-confirm-selected">선택확인</a>
			<div class="right">
...
<script src="/js/page/cs.cancel.js?v=200324"></script>
<link rel="stylesheet" href="/css/sumoselect.min.css">
<script>
	window.name = 'seller_cancel_list';
	CSCancel.CSCancelListInit();

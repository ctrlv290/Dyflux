...
var ProductCommission = (function() {
	...
	var ProductCommissionListInit = function(){
		...
		$("#grid_list").jqGrid({
			...
			colModel: [
				...
				{label: 'comm_idx', name: 'comm_idx', index: 'comm_idx', width: 100, sortable: false, hidden: true},
				...
				//
				// }},
				{label: '구분', name: 'comm_type', index: 'comm_type', width: 40, sortable: false, cellattr:jsFormatterCompareAndSetRowSpan,formatter: function(cellvalue, options, rowobject){
					if(cellvalue == 'NORMAL'){
						var type = '기본';
					}else{
						var type = '행사';
					...
					// return type;
					var btnz = '';
					btnz = '<a href="javascript:;" class="link btn-product-commission-modify" data-comm_idx="'+rowobject.comm_idx+'">'+type+'</a>'
					return btnz;
				}},
				{label: '판매단가', name: 'event_unit_price', index: 'event_unit_price', width: 100, sortable: false, cellattr:jsFormatterCompareAndSetRowSpan,  formatter: function(cellvalue, options, rowobject){
				...
				{ label: '수수료관리', name: 'btn_action', index: 'btn_action', width: 150, sortable: false, cellattr:jsFormatterCompareAndSetRowSpan, formatter: function(cellvalue, options, rowobject){
						var btnz = '';
	...
	var chkcell2 = {cellId:undefined, chkval:undefined, chktype:undefined, rowNo: 0}; //cell rowspan 중복 체크
	...
	var jsFormatterComparePrimaryKey = function(rowid, val, rowObject, cm, rdata){
		...
		if(chkcell.chkval != rowObject.comm_idx && rowid != chkcell.rowNo){ //check 값이랑 비교값이 다른 경우
			...
			//alert(result);
			chkcell = {cellId:cellId, chkval:rowObject.comm_idx, chkname:rowObject.seller_name, rowNo: rowid};
		}else{
	...
	var ProductCommissionWritePopInit = function(){
		...
		$("#grid_list_pop_target").jqGrid({
			...
			rowNum: Common.jsSiteConfig.jqGridRowListBig[2],
	...
	var bindProductCommissionWriteForm = function () {
		...
		$("form[name='dyForm']").submit(function(){
			...
			try{
				...
				if($('input:radio[id=comm_type_2]').is(':checked')) {
					if (!valForm.chkValue(objForm.event_unit_price, "판매단가를 입력해주세요.", 1, 6, null)) return returnType;
				}

...
var StockProduct = (function() {
	...
	var StockProductListInit = function(){
		...
		$('.supplier_idx').on('sumo:opening', function () {
			...
			$('.select-all').children('label').text('전체 공급처');
		});
	...
	var StockListXlsDown = function(){
		...
		var url = "stock_list_xls_down.php?"+$.param(dataObj);
				...
				, formatter: function (cellvalue, options, rowobject) {
					if(cellvalue != 0) {
						return '<a href="javascript:;" class="link btn-stock-detail" ' +
							...
							Common.addCommas(Math.abs(cellvalue)) + '</a>';
					} else {
						return '-';
					}
				}
		...
		$("#grid_list").jqGrid({
			...
			loadComplete: function() {
				...
				$(".btn-stock-detail").on("click", function(){
					StockDailyDetailPopup($(this).data("product_option_idx"),$(this).data("confirm_date"),$(this).data("stock_unit_price"),$(this).data("stock_kind"),$(this).data("stock_status"));
				});
			},
			resizeStop: function(newwidth, index){
				//컬럼 사이즈 저장
     ...
     * @constructor
     */
    var StockDailyDetailPopup = function(product_option_idx, confirm_date, stock_unit_price, stock_kind, stock_status){
        Common.newWinPopup(
    ...
    var StockDailyDetailInit = function(){
        ...
        if($("#stock_kind").val()=="IN"){
            //입고 데이터 컬럼 정의
            var colModel = [
                { label: '발주번호', name: 'stock_order_idx', index: 'stock_order_idx', width: 60, sortable: false, formatter: function(cellvalue, options, rowobject){
						if(rowobject.stock_kind == 'BACK'){
							return '회수';
						} else {
							return cellvalue;
						}
                ...
                { label: '생성일', name: 'stock_request_date', index: 'stock_request_date', width: 120, sortable: true, formatter: function(cellvalue, options, rowobject){
                        return Common.toDateTimeOnlyDate(cellvalue);}},
                { label: '입고예정일', name: 'stock_due_date', index: 'stock_due_date', width: 120, sortable: true},
                { label: '입고일', name: 'stock_in_date', index: 'stock_in_date', width: 120, sortable: true},
                ...
                { label: '메모', name: 'stock_msg', index: 'stock_msg', width: 200, align: 'left', sortable: false, formatter: function(cellvalue, options, rowobject){
            ...
            var colModel = [
				...
				{ label: '주소', name: 'receive_addr1', index: 'O.receive_addr1', width: 240, sortable: false, formatter: function(cellvalue, options, rowobject){
					...
					} else if(Common.isEmpty(cellvalue)){
						return '';
					} else {
						return cellvalue;
					}

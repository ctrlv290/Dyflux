...
var Product = (function() {
	...
	//상품 옵션 목록 바인딩 jqGrid
	var ProductOptionListGridInit = function(){
		var _colModel = [
		...
		if(!isDYLogin) {
			...
			$.each(_colModel, function (i, o) {
				...
				if(typeof o.price != "undefined") {
					...
					if(o.name != "product_option_warning_count" && o.name != "product_option_danger_count" && o.name != "product_option_purchase_price" && o.name != "product_option_stock_btn")
					_colModelVendor.push(o);
				}
			});
		...
		$("#grid_list").jqGrid({
			...
			loadComplete: function(){
				...
				if($("input[name='product_sale_type']").val() == "SELF") {
					$("#grid_list").jqGrid("hideCol", "product_option_purchase_price");
				}else{
					$("#grid_list").jqGrid("hideCol", "product_option_warning_count");
					$("#grid_list").jqGrid("hideCol", "product_option_danger_count");
					$("#grid_list").jqGrid("hideCol", "stock_amount_NORMAL");
					$("#grid_list").jqGrid("hideCol", "product_option_stock_btn");
				}
				...
				$(".btn_sold_out_memo").on("click", function(){
					...
					Common.openCommonInputModalPop('품절 메모 변경', 'update_option_sold_out_memo', '/product/product_option_proc.php', data, ProductOptionListSearch);
				});
				...
				$(".btn_barcode_GTIN").on("click", function(){
					...
					Common.openCommonInputModalPop('바코드(GTIN) 변경', 'update_option_barcode_GTIN', '/product/product_option_proc.php', data, ProductOptionListSearch);
				});
	...
	//상품 옵션 목록/검색
	var ProductOptionListSearch = function(){
		$("#grid_list").setGridParam({
			datatype: "json",
			page: 1,
			postData:{
	...
	var ProductOptionDeletePop = function(product_option_idx){
		var p_url = "product_option_delete.php";
		var dataObj = new Object();

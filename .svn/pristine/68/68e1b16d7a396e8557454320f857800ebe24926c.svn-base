...
if($mode == "add") {
...
} elseif ($mode == "write_manage_by_seller") {
	global $GL_Member;
	...
	$data["product_option_idx"] = $product_option_idx;

	$data["seller_idx"] = $_POST["seller_idx"];
	$data["sale_unit_price"] = $_POST["sale_unit_price"];
	$data["sale_delivery_fee"] = $_POST["sale_delivery_fee"];

	$pdt = new Product();
	$sale_data = $pdt->getSpecialSaleData($data["seller_idx"], $data["product_option_idx"]);
	if($sale_data) {
		$response["msg"] = "이미 해당 판매처에 상품 옵션이 등록되어 있습니다. 검색 후, 수정해주세요.";
		...
		if($pk) {
			$data["idx"] = $_POST["idx"];
			$data["mod_date"] = "NOW()";
			$data["mod_member"] = $GL_Member["member_idx"];
		} else {
			$data["reg_date"] = "NOW()";
			$data["reg_member"] = $GL_Member["member_idx"];
		}

		$rst = $C_Product->insertFromArray($data, "DY_PRODUCT_SPECIAL_VALUE", $pk);

		if ($rst) {
			$response["result"] = true;
			$response["msg"] = "반영되었습니다.";
		}
	}

} elseif ($mode == "delete_manage_by_seller") {
	global $GL_Member;
	...
	$rst = $C_Product->insertFromArray($data, "DY_PRODUCT_SPECIAL_VALUE", "idx");

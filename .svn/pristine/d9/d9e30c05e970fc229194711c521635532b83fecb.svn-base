...
class Report extends DBConn
{
	...
	{
		$addColumn = ", IFNULL(S.seller_name,SU.supplier_name) as target_name";
		$addTable = " Left Outer Join DY_SELLER S On S.seller_idx = R.target_idx 
					  Left Outer Join DY_MEMBER_SUPPLIER SU On SU.member_idx = R.target_idx";
//		$addColumn = "";
//		$addTable = "";
...
//		}
	...
	public function getReportSumDataByAccount($tran_date, $tran_inout, $period_type = "day")
	{
		...
		if($period_type == "day") {
			$qry = "
				...
				From
			     (
			       Select
			              account_idx
			              , SUM(ABS(tran_amount)) as tran_amount
			...
			if($tran_inout == "OUT") {
				$qry = "
					 Select
			            SUM(ABS(tran_amount)) as tran_amount_sum
					   From DY_SETTLE_REPORT
	...
	public function saveReportData($tran_idx, $tran_date, $tran_type, $tran_inout, $account_idx, $tran_memo, $tran_amount, $target_idx, $tran_user, $tran_card_no, $tran_purpose, $tran_is_sync, $tran_sync_idx = null)
	{
		...
		if($tran_idx){
			...
			}elseif($tran_type == "BANK_CUSTOMER_OUT" || $tran_type == "CASH_CUSTOMER_OUT") {
				//통장입출금내역>지출(거래처별)
				$is_ledger = true;
	...
	public function deleteReportData($tran_idx){
		...
		if($_view) {
			...
			$qry = "
				Update DY_SETTLE_REPORT
					Set tran_is_del = N'Y', tran_moddate = NOW(), tran_modip = N'$modip', tran_modidx = N'$last_member_idx'
			    Where tran_idx = N'$tran_idx'
			";
			...
			if($charge_idx) {
				...
				$qry  = "
					...
					Set charge_is_del = N'Y', charge_moddate = NOW(), charge_modip = N'$modip', charge_modidx = N'$last_member_idx'
	...
	/**
	 * 판매처인지 공급처인지 타입 반환 (report_pop)
	 * out : Array
	 */
	public function findSellerSupplierUnionType($target_idx)
	{
		$qry = "
			SELECT type FROM 
			(
			...
			UNION ALL
			(
				SELECT S.member_idx AS idx 
						,S.supplier_name AS NAME
						, 'supplier' AS type
			   From DY_MEMBER_SUPPLIER S 
			)
		)T
		where idx = N'$target_idx';
		";
		parent::db_connect();

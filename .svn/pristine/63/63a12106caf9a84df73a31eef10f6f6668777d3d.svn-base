...
if($_search_param)
{
	...
	if(
		...
		if($_search_paramAryList["stock_kind"] == "IN"){
			...
			$_qry_stock_kind = "
				And (stock_kind = N'STOCK_ORDER' Or stock_kind = N'ORDER')
				And stock_type = 1
...
$args['qry_table_name'] 	.= " 
									...
									From 
										(
											Select *, convert(date, stock_is_confirm_date) as confirm_date From DY_STOCK 
											Where 
											stock_is_del = N'N' 
									...
									On DateTable.value = STOCK_IN.confirm_date

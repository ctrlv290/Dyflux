...
if($_search_param)
{
	...
	if($_search_paramAryList["supplier_idx"]){
		$qryWhereAry[] = "a.supplier_idx = N'" . $_search_paramAryList["supplier_idx"] . "'";
	}
}
...
// make select query
$args['qry_table_idx'] 	= "SUPPLIER_IDX";
$args['qry_get_colum'] 	= "*";
$args['qry_table_name'] 	= " 
                                (
								WITH stock_in AS 
								...
								Select
									...
									, IFNULL(Sum(stock_unit_price * stock_amount), 0) as stock_out_sum
									, IFNULL(Sum(stock_amount), 0) as stock_out_amount
									, IFNULL(Sum(Case 
                                ...
                                FROM  stock_in
                                LEFT JOIN stock_out                                                                  
                                    ON stock_in.i_supplier_idx = stock_out.o_supplier_idx 
                                LEFT OUTER JOIN dy_member_supplier s 
                                    ON s.member_idx = stock_in.i_supplier_idx 
                                    OR s.member_idx = stock_out.o_supplier_idx                             
                                UNION                               
                                SELECT          
                                    STOCK_IN.*
                                    , STOCK_OUT.*

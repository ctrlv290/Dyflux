...
class Vendor extends DBConn
{
	...
	public function insertVendor($args)
	{
		...
		if($maxIDX)
		{
			$qry = "
				...
				vendor_email_default, vendor_email_account, vendor_email_order, vendor_use_charge, vendor_is_order_block,
	...
	public function updateVendor($args)
	{
		...
		if($vendorInfo) {
			...
			if ($vendor_is_order_block != null) {
				$qry .= "
					vendor_is_order_block = N'$vendor_is_order_block',
				";
			}
	...
	public function getVendorRemainChargeAmount($vendor_idx)
	{
		...
		$qry = "
			...
			Sum(charge_amount * charge_inout) 
			  ...
			  + (Select IFNULL(Sum(ledger_tran_amount), 0) - IFNULL(Sum(ledger_adjust_amount), 0) From DY_LEDGER L Where L.target_idx = N'$vendor_idx' And L.ledger_is_del = N'N' And L.charge_idx = 0)
			  as charge_sum

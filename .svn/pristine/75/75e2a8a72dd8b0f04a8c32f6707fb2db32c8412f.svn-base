...
$qry = "
    select idx, tp_code, send_phone, al_date from DY_SMS_AL_SEND_RESULT where convert(nvarchar, al_date,120) between '$before_date' AND '$now_date' 
";
//echo $qry;
$C_Dbconn->db_connect();
...
for($i=0; $i<count($data); $i++) {
    ...
    $qry = "
        select count(*) as cnt from [DYFLUX.SMS].dbo.BIZ_LOG_" . $table_month . " where REQUEST_TIME='$al_date' and TEMPLATE_CODE='$tp_code' and SEND_PHONE='$send_phone' AND CALL_STATUS='7000' 
    ";
    //echo $qry;

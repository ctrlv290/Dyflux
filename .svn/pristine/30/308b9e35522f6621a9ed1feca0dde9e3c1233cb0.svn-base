...
if($_search_param)
{
	...
	if($_search_paramAryList["date"] && $_search_paramAryList["time_start"] && $_search_paramAryList["time_end"]){
		$qryWhereAry[] = " 
				A.shipping_date >= '".$_search_paramAryList["date"]." ".$_search_paramAryList["time_start"]."'
				And A.shipping_date <= '".$_search_paramAryList["date"]." ".$_search_paramAryList["time_end"]."'
		";
	}
}
...
";
$args['qry_where']			= " A.order_is_del = N'N' And A.order_progress_step = N'ORDER_SHIPPED' ";

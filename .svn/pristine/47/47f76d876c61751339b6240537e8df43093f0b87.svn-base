...
$ret_json = array(
	...
	'status_code' => 0,
	'result_text' => '',
	'request_cnt' => 0,    // 전체 송장 등록 요청 수
	'confirm_cnt' => 0,    // 송장 등록 승인 수
	'reject_cnt' => 0,     // 송장 등록 거부 수 (이미 송장 이상으로)
...
$s_date     = $_GET["s_date"];
$e_date     = $_GET["e_date"];
$invoice_orders     = $_GET["cc_orders"];

if (!$s_date || !$e_date || !$mk_code) {
	$ret_json['status_code'] = -9999;
	$ret_json['result_text'] = "날짜나 마켓 코드가 없습니다.";
...
switch ($market_code) {
	...
	default:
		...
		if($invoice_orders != "") {
			...
			$row_num = 0;
			$cancel_orders_list = explode("|", $invoice_orders);
...
//					echo $seller_idx;
					...
					if (count($_row) > 0) {
						...
						if($_row["order_progress_step"] == "ORDER_SHIPPED") {
							...
							$ret['deliveryInfo'][$row_num]["invoice_no"] = str_replace("-", "", $_row["invoice_no"]);

...
$args['qry_get_colum'] 	= " 
							A.*
							, A.receive_addr1 + ' ' + A.receive_addr2 as receive_addr
...
$args['qry_table_name'] 	= " DY_ORDER A WITH (NOLOCK)
								Left Outer Join (
									Select 
										...
										,STUFF((
											SELECT Distinct ',' + CONVERT(VARCHAR(1000), C2.code_name)
											FROM 
												DY_ORDER O2 WITH (NOLOCK)
												Left Outer Join DY_ORDER_PRODUCT_MATCHING AS OPM2 WITH (NOLOCK) ON O2.order_idx = OPM2.order_idx And OPM2.order_matching_is_del = N'N'
												Left Outer Join DY_CODE C2 WITH (NOLOCK) On C2.parent_code = N'ORDER_MATCHING_CS' And C2.code = OPM2.order_cs_status
											WHERE O2.order_pack_idx = OO.order_pack_idx
											FOR XML PATH('')
											), 1, 1, '') AS order_cs_status_list
										
									From DY_ORDER OO WITH (NOLOCK)
									Left Outer Join DY_ORDER_PRODUCT_MATCHING OOPM WITH (NOLOCK) On OO.order_idx = OOPM.order_idx And OOPM.order_matching_is_del = N'N'
									Where OO.order_is_del = N'N'
									Group by order_pack_idx
								...
								Left Outer Join (
									Select 
									    ...
									    , Sum(isNull((Case When cs_is_auto = 'N' Then 1 Else 0 End), 0)) as cs_manual_cnt
								    From DY_ORDER_CS CCC WITH (NOLOCK)
								    Where cs_is_del = 'N'
								    Group by order_pack_idx
...
if(count($qryWhereOrderSearch) > 0)
{
	$args["qry_where"] .= "	And A.order_idx in (
						Select order_pack_idx 
							From DY_ORDER pO WITH (NOLOCK)
								Left Outer Join DY_ORDER_PRODUCT_MATCHING pM WITH (NOLOCK) On pO.order_idx = pM.order_idx And pM.order_matching_is_del = N'N' 
								Left Outer Join DY_PRODUCT_OPTION pPO WITH (NOLOCK) On pM.product_option_idx = pPO.product_option_idx And pPO.product_option_is_del = N'N'
								Left Outer Join DY_PRODUCT pP WITH (NOLOCK) On pP.product_idx = pPO.product_idx And pP.product_is_del = N'N'

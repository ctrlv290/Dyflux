...
foreach($fields as $row) {
	...
	echo "
		INSERT INTO $history_table_name ([table_nm],[table_idx1],[column_mn],[before_data],[after_data],[member_idx],[dml_flag],[memo]) 
		(SELECT '$target_table_name', I.[$target_table_idx_column], '$column_name', '', I.[$column_name], I.[last_member_idx], 'I','$column_desc' FROM inserted I WHERE IFNULL(I.[$column_name], '') <> '' ); 
	";
}
...
foreach($fields as $row) {
	...
	echo "
		INSERT INTO $history_table_name ([table_nm],[table_idx1],[column_mn],[before_data],[after_data],[member_idx],[dml_flag],[memo])
		(SELECT '$target_table_name', [$target_table_idx_column],'$column_name',D.[$column_name],'', D.[last_member_idx], 'D','$column_desc' FROM deleted D WHERE IFNULL(D.[$column_name], '') <> '' ); 
	";
}

...
$args['qry_table_name'] 	= " DY_BBS A 
								...
								Left Outer Join DY_CODE CC On CC.parent_code = N'BBS_BIZ_CATEGORY' And CC.code = A.bbs_category
...
";
$args['qry_where']			= " A.bbs_is_del = N'N' And A.bbs_id = 'biz'";
...
//업체게시판 카테고리 가져오기
$C_Code = new Code();
$_biz_category_list = $C_Code -> getSubCodeList("BBS_BIZ_CATEGORY");
	...
	<div class="content">
		<div class="w900px">
			<div class="wrap_tab_menu">
				<ul class="tab_menu">
					<li><a href="biz_list.php" class="<?=($bbs_category == "") ? "on" : ""?>">전체</a></li>
					<?php foreach($_biz_category_list as $cate){?>
					<li><a href="biz_list.php?category=<?=$cate["code"]?>" class="<?=($bbs_category == $cate["code"]) ? "on" : ""?>"><?=$cate["code_name"]?></a></li>
					<?php }?>
				</ul>
			</div>
			...
			<div>
				<?php if($canWrite){?>
					<a href="biz_write.php" class="btn btn-write">신규등록</a>
				<?php }?>
			</div>
			<div class="tb_wrap">
				<table>
						...
						foreach($_top_list as $_top){
							...
							<tr>
								...
								<td class="text_left ellipsis">
									<a href="biz_view.php?bbs_id=<?=$bbs_id?>&bbs_idx=<?=$_top["bbs_idx"]?>&<?=$page_parameters?>" class="link"><?=htmlentities_utf8($_top["bbs_title"])?></a>
								</td>
								<td><?=$_top["bbs_name"]?></td>
					...
					foreach($listRst as $row) {
						...
						<tr>
							...
							<td class="text_left ellipsis">
								<?=$level_icon?>
								<a href="biz_view.php?bbs_id=<?=$bbs_id?>&bbs_idx=<?=$row["bbs_idx"]?>&<?=$page_parameters?>" class="link"><?=htmlentities_utf8($row["bbs_title"])?></a>
							</td>
							<td><?=$row["bbs_name"]?></td>

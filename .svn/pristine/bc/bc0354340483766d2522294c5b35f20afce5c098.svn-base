...
class Shorty extends Dbconn
{
	 ...
	 * Hostname
	 */
	private $hostname = DY_SHORTURL_URL;
	 ...
	 * @return bool|string Alpha-numeric string
	 */
	...
	public function fetch($id) {
		...
		$qry = "SELECT * FROM DY_SHORT_URL WHERE id = '$id'";
		parent::db_connect();
	...
	public function find($url) {
		$qry = "Select * From DY_SHORT_URL Where url = '$url'";
		parent::db_connect();
		$rst = parent::execSqlOneRow($qry);
		parent::db_close();
	...
	public function store($url) {
		$qry = "
			Insert Into DY_SHORT_URL
			(url, created) values ('$url', getdate())
		";
	...
	public function update($id) {
		$qry = "
			Update DY_SHORT_URL
			Set hits = hits + 1, accessed = getdate()
			Where id = '$id'
		";

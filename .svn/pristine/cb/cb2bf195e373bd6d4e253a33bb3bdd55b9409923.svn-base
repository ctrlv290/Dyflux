...
if($_search_param)
{
	...
	if($_search_paramAryList["period_type"] == "settle_date"){
		$date_search_col = "settle_date";
		$search_date_col_name = " CASE WHEN T.settle_date != CONVERT(VARCHAR(30),T.settle_regdate,23)
		                               Then CONVERT(VARCHAR(30),T.settle_date,23)
                                       ELSE CONVERT(VARCHAR(30),T.settle_regdate,120) End AS search_date ";
		$date_search_col_name_han = "정산일";
        ...
        $search_date_col_name = " ISNULL(CONVERT(VARCHAR(30),O.order_progress_step_accept_date, 120), 
                                    Case when T.settle_date != CONVERT(VARCHAR(30), T.settle_regdate,23) 
                                    THEN CONVERT(VARCHAR(30),T.settle_date,23)
                                    ELSE CONVERT(VARCHAR(30),T.settle_regdate,120) END
			                      ) AS search_date ";
        ...
        if($_search_paramAryList["date_start"] != "" && $_search_paramAryList["date_end"] != ""){
            $qryWhereAry[] = "	 
			(
				ISNULL($date_search_col ,T.settle_date) >= '".$_search_paramAryList["date_start"]." 00:00:00' 
				And ISNULL($date_search_col ,T.settle_date) <= '".$_search_paramAryList["date_end"]." 23:59:59.998'
			) 
		";
        }
		...
		$search_date_col_name = " ISNULL(CONVERT(VARCHAR(30), O.order_progress_step_accept_temp_date, 120), 
									CASE WHEN T.settle_date != CONVERT(VARCHAR(30),T.settle_regdate,23) 
									THEN ISNULL(CONVERT(VARCHAR(30),O.order_progress_step_accept_date, 120), CONVERT(VARCHAR(30),T.settle_date,23))
									ELSE ISNULL(CONVERT(VARCHAR(30),O.order_progress_step_accept_date, 120), CONVERT(VARCHAR(30),T.settle_regdate,120)) END
		                          ) AS search_date ";
		...
		if($_search_paramAryList["date_start"] != "" && $_search_paramAryList["date_end"] != ""){
			$qryWhereAry[] = "	 
			(
				ISNULL($date_search_col , ISNULL(O.order_progress_step_accept_date,T.settle_date)) >= '".$_search_paramAryList["date_start"]." 00:00:00' 
				And ISNULL($date_search_col , ISNULL(O.order_progress_step_accept_date,T.settle_date)) <= '".$_search_paramAryList["date_end"]." 23:59:59.998'
			) 
		";
		}
	}else{
...
$args['qry_table_name'] 	= "
							...
							, isNull(O.receive_addr1, '') as receive_addr1
							, isNull(O.receive_addr2, '') as receive_addr2
							, isNull(O.receive_zipcode, '') as receive_zipcode
...
//합계
$qry = "Select
	ISNULL(SUM(CASE WHEN settle_type != 'UPLOAD' THEN CAST(settle_sale_supply AS BIGINT) END) 
	- SUM(CASE WHEN settle_type != 'UPLOAD' THEN CAST(settle_sale_commission_in_vat AS BIGINT) END) 
	+ SUM(CASE WHEN settle_type != 'UPLOAD' THEN CAST(settle_delivery_in_vat AS BIGINT) END)
	- Sum(CAST(settle_delivery_commission_in_vat AS BIGINT)), 0) as sale_sum
	, ISNULL(SUM(CASE WHEN settle_type = 'UPLOAD' THEN CAST(settle_sale_sum AS BIGINT) END),0) AS sale_sum_upload
	, ISNULL(SUM(CASE WHEN settle_type != 'UPLOAD' THEN CAST(settle_purchase_supply AS BIGINT) END) 
	+ SUM(CASE WHEN settle_type != 'UPLOAD' THEN CAST(settle_purchase_delivery_in_vat AS BIGINT) END), 0) AS purchase_sum
	, ISNULL(Sum(CASE WHEN settle_type = 'UPLOAD' THEN CAST(settle_purchase_sum AS BIGINT) END),0) AS purchase_sum_upload 
	, ISNULL(Sum(CAST(settle_sale_profit AS BIGINT)), 0) as profit_sum
";

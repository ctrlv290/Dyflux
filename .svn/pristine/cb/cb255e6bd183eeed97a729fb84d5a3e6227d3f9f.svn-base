...
$order_is_block = false;
if (!$isDY) {
	global $GL_Member;
	if ($GL_Member["member_type"] == "VENDOR") {
		$vendor_con = new Vendor();
		$vendor_info = $vendor_con->getVendorData($GL_Member["member_idx"]);
		$order_is_block = $vendor_info["vendor_is_order_block"] === "Y";
    }
}
...
<div class="container">
	...
	<div class="content">
		<div class="step_wrap">
			...
			<?php } ?>
			<?php if($order_is_block) {?>
                <span style="color: red;">충전금 잔액이 부족하여 발주가 제한되었습니다. 담당 부서에 문의주시길 바랍니다.</span>
			<?php } else { ?>
		...
		<div class="grid_btn_set_top">
			...
			<div class="right">
				...
				<?php if($isDY) {?><a href="javascript:;" class="btn red_btn btn-all-order-delete">전체발주삭제</a><?php } ?>
			</div>
		</div>
...
<script src="/js/page/order.order.js?v=200916"></script>
<script>
    ...
    let isOrderBlock = <?php echo $order_is_block ? "true" : "false";?>;

	Order.OrderListInit(isOrderBlock);
</script>

...
$order_by = "PO.product_option_idx DESC";
...
if($_search_param)
{
		...
		if(trim($val) && in_array($col, $available_col)) {
			if(
                ...
                if($val == "product_name_option_name" && trim($_search_paramAryList["search_keyword"]) != "") {
                    $qryWhereAry[] = " ( product_name like N'%" . trim($_search_paramAryList["search_keyword"]) . "%' Or product_option_name like N'%" . trim($_search_paramAryList["search_keyword"]) . "%' )";
                }
				elseif(trim($_search_paramAryList["search_keyword"]) != "") {
	...
	if($_search_paramAryList["search_date"] != ""){
		//입고 확정 된 기간
		$_searchWhereStockProcDate = "	 
				And stock_is_confirm_date <= '".$_search_paramAryList["search_date"]." 23:59:59'
		";
	}
...
$args['qry_get_colum'] 	= " 
							STOCK.*
							, STOCK2.stock_amount_NORMAL, STOCK2.stock_amount_ABNORMAL
							, STOCK2.stock_amount_BAD, STOCK2.stock_amount_HOLD
							, STOCK2.stock_amount_DISPOSAL
							, P.product_img_main
...
$args['qry_table_name'] 	= " 
								...
								Select 
									...
									, Sum(Case When stock_status = 'NORMAL' Then stock_amount * stock_type Else 0 End) as stock_amount_NORMAL_NOW
								From DY_STOCK ST

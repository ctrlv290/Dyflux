...
if(!empty($bbs_idx)) {
	...
	if (!$_ref_view || $_ref_view["bbs_id"] != "work") {
		put_msg_and_back("잘못된 접근입니다.");
	} else {
		$mode = "reply";
		$bbs_step = $bbs_step + 1;
	...
	<div class="content write_page">
		<div class="content_wrap w800px">
			<form name="dyForm" method="post" class="<?php echo $mode?>" action="bbs_proc.php?<?=$page_parameters?>">
				...
				<input type="hidden" name="bbs_id" value="work" />
				...
				<div class="tb_wrap">
					<table>
						...
						<?php }?>
						<?php if(($GL_Member["member_type"] == "USER" || $GL_Member["member_type"] == "ADMIN") && empty($ref_idx)){?>
							<tr>
								<th>공지등록</th>
								<td class="text_left">
									<label>

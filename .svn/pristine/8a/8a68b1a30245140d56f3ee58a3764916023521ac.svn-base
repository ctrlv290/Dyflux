...
class Loan extends DBConn
{
	 ...
	 * @return int
	 */
	public function insertLoanAccount($loan_name, $loan_amount, $loan_detail, $loan_sort, $loan_is_use, $loan_start_date, $loan_use_n_date)
	{
		global $GL_Member;
		...
		$qry = "
			...
			(loan_name, loan_amount, loan_detail, loan_sort, loan_is_use, loan_start_date, loan_use_n_date, loan_regip, loan_regidx) 
			VALUES
			(
			 N'$loan_name'
			 , N'$loan_amount'
	...
	public function getTodayLoanTransaction($date)
	{
		$qry = "
			...
			AND B.loan_start_date <= N'$date'
			AND (B.loan_use_n_date > N'$date' OR B.loan_use_n_date = N'0000-00-00')
	...
	public function getTodayLoanTransactionDetail($period_type, $date)
	{
		...
		if($period_type == "day") {
			$qry = "
				...
				Where 1 = 1
				AND BANK.loan_is_del = N'N'
				AND BANK.loan_start_date <= N'$date'

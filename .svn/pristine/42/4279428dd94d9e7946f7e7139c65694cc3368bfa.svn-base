...
$args['qry_get_colum'] 	= " A.product_option_idx, A.product_idx
							...
							, P.product_sale_type
							, IFNULL(STOCK.stock_amount_NORMAL, 0) as stock_amount_NORMAL
							, IFNULL(A.product_option_soldout_memo, N'') AS product_option_soldout_memo
							, IFNULL(A.product_option_barcode_GTIN, N'') AS product_option_barcode_GTIN
                            ";
...
if (!isDYLogin()) {
	...
	if ($GL_Member["member_type"] == "VENDOR") {
		$vendor_idx = $GL_Member["member_idx"];

		$added_col = ", PSV.sale_unit_price AS special_sale_unit_price";
		$added_join = "LEFT JOIN DY_PRODUCT_SPECIAL_VALUE PSV ON A.product_option_idx = PSV.product_option_idx AND PSV.seller_idx = $vendor_idx";

		$args['qry_get_colum'] .= $added_col;
		$args['qry_table_name'] .= $added_join;
	}
}
...
$WholeGetListResult = $list_table_con -> WholeGetListResult($args);

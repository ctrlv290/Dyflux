...
if($_GET["supplier_idx"]){
	$qryWhereAry[] = " product_option_idx in (Select product_option_idx From DY_PRODUCT_OPTION Where product_option_is_del = N'N' And supplier_idx = N'".$_GET["supplier_idx"]."')";
}

//판매처
if($_GET["seller_idx"]){
	$qryWhereAry[] = " seller_idx = N'".$_GET["seller_idx"]."'";
}
$qryWhere = "";
if (count($qryWhereAry) > 0) {
	$qryWhere = " And " . join(" And ", $qryWhereAry);
}
...
$qry .= "
	...
	From (
		...
		(
			Select S.*
			From DY_SETTLE S
				LEFT OUTER JOIN DY_PRODUCT_OPTION P ON S.product_option_idx = P.product_option_idx
			Where 
				settle_is_del = N'N'
		    	And settle_date >= N'$date_start' 
		    	And settle_date <= N'$date_end'
				$qryWhere
		) ST
		Group by product_category_l_idx, product_category_m_idx with rollup
	...
	Order by C1.sort, Case When C2.c2_sort IS NULL Then 10000 Else C2.c2_sort End, rowNum
";

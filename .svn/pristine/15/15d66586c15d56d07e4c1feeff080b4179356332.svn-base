...
$args['qry_get_colum'] 	= " 
							...
							, O.order_idx, O.market_order_no , O.invoice_no
							, O.delivery_code
							, (Select delivery_name From DY_DELIVERY_CODE D Where D.delivery_code = O.delivery_code LIMIT 1) as delivery_name
							...
							, CONCAT(O.receive_addr1, ' ', O.receive_addr2) as receive_addr
...
//포맷설정 불러오기
$format_qry ="
                SELECT FD.order_download_format_default_header_code
                        , FD.order_download_format_default_width
                        , FD.order_download_format_default_data_type
                        , FS.order_download_format_header_name
                FROM DY_ORDER_DOWNLOAD_FORMAT_SUPPLIER FS
                LEFT JOIN DY_ORDER_DOWNLOAD_FORMAT_DEFAULT FD ON FS.order_download_format_default_idx = FD.order_download_format_default_idx
                WHERE FS.supplier_idx = N'$_supplier_idx' 
                AND FS.order_download_format_is_del =N'N'
            ";

$C_Dbconn -> db_connect();
$format_list = $C_Dbconn -> execSqlList($format_qry);
$C_Dbconn -> db_close();

//포맷설정 있을 시
if($format_list) {
    $xls_header = array();
    foreach ($format_list as $row) {
        array_push($xls_header,
            array(
                "header_name" => $row["order_download_format_header_name"],
                "field_name" => $row["order_download_format_default_header_code"],
                "width" => $row["order_download_format_default_width"],
            )
        );
    }
}
...
foreach($_list as $row_num => $row) {
	...
	foreach ($xls_row as $cellValue) {
		...
		if($field_name == "product_option_purchase_price"){
			$cellValue = $row["product_option_purchase_price"];
		}elseif($field_name == "delivery_fee"){
			if($row["inner_no"] == "1"){
		...
		}elseif($field_name == "order_sum1"){
            $cellValue = ($row["product_option_purchase_price"] * $row["product_option_cnt"]);
        }elseif($field_name == "null_header"){
            $cellValue = "";
        }elseif($field_name == "product_name_option_name"){
            $cellValue = $row["product_name"]." / ".$row["product_option_name"];
        }elseif($field_name == "invoice_name" || $field_name == "invoice_tel"){
            $C_SiteInfo = new SiteInfo();
            $_view = $C_SiteInfo->getSiteInfo();
            if($_view){
                extract($_view);
                if($field_name == "invoice_name") {
                    $cellValue = $invoice_name;
                }elseif ( $field_name == "invoice_tel"){
                    $cellValue = $invoice_tel;
                }
            }else{
                $cellValue = "";
            }

...
if($mode == "new_order_write") {
	...
	$rst = $C_CS->restoreOrderByPackage($order_pack_idx, $cs_msg);
	...
	$rst = $C_CS->restoreOrderByMatching($order_matching_idx, $cs_msg);
	...
	if($rst) {
		...
		$response["result"]         = false;
		$response["order_pack_idx"] = $_POST["order_pack_idx"];
		$response["msg"]            = "오류가 발생하였습니다. 개별 정상 복귀에 실패하였습니다.";
	}
	...
	$send_info                 = array();
	$send_info["send_name"]    = $_POST["send_name"];
	$send_info["send_tel_num"] = $_POST["send_tel_num"];
	$send_info["send_hp_num"]  = $_POST["send_hp_num"];
	$send_info["send_zipcode"] = $_POST["send_zipcode"];
	$send_info["send_address"] = $_POST["send_address"];
	$send_info["send_memo"]    = $_POST["send_memo"];

	$receive_info                    = array();
	$receive_info["receive_name"]    = $_POST["address_name"];
	$receive_info["receive_tel_num"] = $_POST["address_tel_num"];
	$receive_info["receive_hp_num"]  = $_POST["address_hp_num"];
	$receive_info["receive_zipcode"] = $_POST["address_zipcode"];
	$receive_info["receive_address"] = $_POST["address_address"];

	$request_info                         = array();
	$request_info["delivery_pay_type"]    = $_POST["delivery_pay_type"];
	$request_info["delivery_return_type"] = $_POST["delivery_return_type"];
	$request_info["box_num"]              = $_POST["box_num"];
	$request_info["product_price"]        = $_POST["product_price"];
	$request_info["delivery_price"]       = $_POST["delivery_price"];
	$request_info["pay_site"]             = $_POST["pay_site"];
	$request_info["pay_pack"]             = $_POST["pay_pack"];
	$request_info["pay_account"]          = $_POST["pay_account"];

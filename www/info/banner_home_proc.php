...
if($mode == "add") {
	...
	if(in_array($mimetype, array('image/jpeg', 'image/gif', 'image/png'))) {
		if ($_FILES["banner_image"]["error"] > 0) {
			...
			put_msg_and_go("등록되었습니다.", "banner_home_list.php");
	...
	put_msg_and_go("수정되었습니다.", "banner_home_list.php");
}elseif($_GET["mode"] == "del" && $_GET["banner_idx"]) {

	$tmp = $C_Banner->deleteBanner($_GET["banner_idx"]);
	put_msg_and_go("삭제되었습니다.", "banner_home_list.php");
	...
	//print_r2($rst);
	if($rst["result"]){
		go_replace("banner_home_list.php");
	}else{

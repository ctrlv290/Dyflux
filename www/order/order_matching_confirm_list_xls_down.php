...
$_SESSION["MATCHING_CONFIRM_LIST"] = "Y";
...
$xls_header = array(
	...
	array(
		"header_name" => "판매처 옵션",
		"field_name" => "market_product_option",
		"width" => 30,
	),
	array(
		"header_name" => "매칭정보",
		"field_name" => "matching_info",
		"halign" => "left",
		"width" => 60,
	),
);
...
foreach($_list as $row_num => $row) {
	...
	$matching_info    = str_replace("[;;]", PHP_EOL, $matching_info);
	...
	foreach ($xls_row as $cellValue) {
		...
		if($field_name == "matching_info"){
			$cellValue = $matching_info;

			$spreadsheet->getActiveSheet()->getStyle($cod)->getAlignment()->setWrapText(true);
		}
...
ob_end_flush();
$_SESSION["MATCHING_CONFIRM_LIST"] = "";

...
if($file_idx)
{
	...
	$_file_view = $C_Settle -> getLedgerDownloadFileLog($file_idx);
	$filename = $_file_view["file_name"];
}

$down_filename_prefix = "";
if($_file_view["ledger_type"] == "LEDGER_PURCHASE"){
	$down_filename_prefix = "매입원장";
}else{
	$down_filename_prefix = "매출원장";
}

if($_file_view) {
	$filepath      = DY_SETTLE_PATH . "/" . $filename;
	...
	if (file_exists($filepath)) {
		...
		if (isset($_GET["email_idx"])) {
			$C_Settle->insertLedgerDocumentDownLog($target_idx, $file_idx, $_GET["email_idx"]);
		}

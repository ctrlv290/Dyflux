...
var CSPopup = (function() {
	...
	var CSPopupTabGridInit = function(order_pack_idx, order_progress_step){
		...
		$(".tab_btn_set_"+order_pack_idx+" .btn-cs-order-return").on("click", function(){
			var _order_pack_idx = order_pack_idx;
			CSPopupOrderReturnOpen(_order_pack_idx);
		});
		//재고회수
		$(".tab_btn_set_"+order_pack_idx+" .btn-cs-stock-return").on("click", function(){
			var _order_pack_idx = order_pack_idx;
			CSPopupStockReturnOpen(_order_pack_idx);
		});
	 ...
	 * @constructor
	 */
	var CSPopupOrderRestoreAllOpen = function(){
		...
		var p_url = "cs_pop_restore_all.php";
		var dataObj = new Object();
		dataObj.order_pack_idx = _CSPopupTabCurrentTabOrderPackIdx;
		showLoader();
		$.ajax({
	 ...
	 * @constructor
	 */
	var CSPopupOrderRestoreAllInit = function(){
		//창 닫기 버튼 바인딩
		$(".btn-common-pop-close").on("click", function(){
			CSPopupCommonPopClose();
		});
	 ...
	 * @constructor
	 */
	var CSPopupOrderRestoreOneOpen = function(){
		//현재 상태가 접수, 배송 상태만 가능
		if(_CSPopupCurrentTabInfoAry[_CSPopupTabCurrentTabOrderIdx].order_progress_step != "ORDER_ACCEPT"
		&& _CSPopupCurrentTabInfoAry[_CSPopupTabCurrentTabOrderIdx].order_progress_step != "ORDER_SHIPPED"){
		...
		if(!CSPopupTabCheckBoxCalculate("restore_one")){
			return;
		}

		var p_url = "cs_pop_restore_one.php";
		var dataObj = new Object();
		dataObj.order_pack_idx = _CSPopupTabCurrentTabOrderPackIdx;
		showLoader();
		$.ajax({
	 ...
	 * @constructor
	 */
	var CSPopupOrderRestoreOneInit = function(){
		//창 닫기 버튼 바인딩
		$(".btn-common-pop-close").on("click", function(){
			CSPopupCommonPopClose();
		});
	 ...
	 * @param chk_type : 체크 타입 (합포제외 : except, 개별취소 : cancel_one, 개별정상복귀 : restore_one
	 * @param chk_type
	 * @returns {boolean}
	 * @constructor
	 */
	var CSPopupTabCheckBoxCalculate = function(chk_type){
		...
		}else if(chk_type == "cancel_one") {

		}else if(chk_type == "restore_one") {
	...
	/**
	 * CS 팝업 페이지 - 재고회수 팝업 Open
	 * @constructor
	 */
	
	var CSPopupStockReturnOpen = function(){
		//현재 상태가 배송 상태일 아닐 경우 변경 불가
		if(_CSPopupCurrentTabInfoAry[_CSPopupTabCurrentTabOrderIdx].order_progress_step != "ORDER_SHIPPED") {
			alert("재고회수는 배송된 주문만 가능합니다.");
			return;
		}
		if(_CSPopupCurrentTabInfoAry[_CSPopupTabCurrentTabOrderIdx].order_cs_status != "ORDER_CANCEL") {
			alert("재고회수는 취소된 주문만 가능합니다.");
			return;
		}
		if(_CSPopupCurrentTabInfoAry[_CSPopupTabCurrentTabOrderIdx].product_sale_type != "SELF") {
			alert("재고회수는 사입상품만 가능합니다.");
			return;
		}
		var url = '/cs/cs_pop_stock_return.php?order_idx='+_CSPopupTabCurrentTabOrderIdx;
	 ...
	 * @constructor
	 */
	var CSPopupStockReturnInit = function(order_idx){
	 ...
	 * @constructor
	 */
	var CSPopupOrderReturnOpen = function(){
		...
		$.ajax({
			...
			if(response)
			{
				CSPopupCommonPopOpen(1300, 0, "회수", response);
			}else{
				alert('오류가 발생하였습니다. 잠시 후 다시 시도하여 주세요.');
			}
			hideLoader();
		...
		*/
		...
		var url = '/cs/cs_pop_return.php?order_idx='+_CSPopupTabCurrentTabOrderIdx;
	 ...
	 * @constructor
	 */
	var CSPopupOrderReturnRequestList = new Object();
	var CSPopupOrderReturnInit = function(order_idx){
		...
		CSPopupOrderReturnRequestList = new Object();
	 ...
	 * @constructor
	 */
	var CSPopupOrderReturnRequest = function(){
		var _s = CSPopupOrderReturnFormValue("add");
	...
	var CSPopupOrderReturnFormValue = function(chk_type){
		var _s = new Object();

		_s = CSPopupOrderReturnAddressBookReceiveFormValue();

		var tel1 = $(".cs_order_return_popup input[name='send_tel_num1']").val();
		var tel2 = $(".cs_order_return_popup input[name='send_tel_num2']").val();
		var tel3 = $(".cs_order_return_popup input[name='send_tel_num3']").val();

		var hp1 = $(".cs_order_return_popup input[name='send_hp_num1']").val();
		var hp2 = $(".cs_order_return_popup input[name='send_hp_num2']").val();
		var hp3 = $(".cs_order_return_popup input[name='send_hp_num3']").val();
	...
	return {
		...
		CSPopupOrderReturnInit: function(order_pack_idx){
			CSPopupOrderReturnInit(order_pack_idx)
		},
		CSPopupStockReturnInit: function(order_pack_idx){
			CSPopupStockReturnInit(order_pack_idx)
		},

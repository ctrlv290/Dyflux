	...
	<table class="ledge" data-date="<?=$month?>">
		...
		<?php
			...
			if ($stock_order_total = $C_Settle->getStockOrderAmountForDate($supplier_idx, $row["dt"])) {
				...
				<tr class="detail">
					<td></td>
					<td class="text_left"><a class="link" href="/stock/stock_confirm_list.php?period_type=stock_confirm_date&supplier_idx=<?=$supplier_idx?>&date_start=<?=$row["dt"]?>&date_end=<?=$row["dt"]?>" target="_blank">사입 발주 확정</a></td>
					<td class="text_right minimize"></td>

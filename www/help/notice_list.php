...
";
$args['qry_where']			= " A.bbs_is_del = N'N' And A.bbs_id = 'notice'";
...
if(!isDYLogin()){
	if($GL_Member["member_type"] == "VENDOR"){
		$args['qry_where'] .= " And ( A.bbs_target = N'ALL' Or A.bbs_target = N'SELLER' ) ";
	}elseif($GL_Member["member_type"] == "SUPPLIER"){
		$args['qry_where'] .= " And ( A.bbs_target = N'ALL' Or A.bbs_target = N'SUPPLIER' ) ";
	}
}
...
<div class="container">
	...
	<div class="content">
		<div class="w900px">
			...
			<div>
				<?php if($canWrite){?>
				<a href="notice_write.php" class="btn btn-write">신규등록</a>
				<?php }?>
			</div>
			<div class="tb_wrap">
				<table>
					...
					<tbody>
						...
						<tr>
							...
							<td class="text_left ellipsis">
								<a href="notice_view.php?bbs_id=<?=$bbs_id?>&bbs_idx=<?=$row["bbs_idx"]?>&<?=$page_parameters?>" class="link"><?=htmlentities_utf8($row["bbs_title"])?></a>
							</td>

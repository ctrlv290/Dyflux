...
//검색 가능한 컬럼 지정
$available_col = array(
	"order_seller_cancel_off_confirm",
	"search_column",
);
...
$available_search_col = array(
	...
	"order_name",
	"receive_name",
	"order_seller_cancel_off_confirm_member_id"
);
...
if($_search_param)
{
	...
	foreach($_search_paramAry as $sitem) {
		...
		if(trim($val) && in_array($col, $available_col)) {
			if(
				$col == "order_seller_cancel_off_confirm"
			) {
				$qryWhereAry[] = $col . " = N'" . $val . "'";
			}elseif($col == "search_column" && in_array($val, $available_search_col)){

				if($val == "order_seller_cancel_off_confirm_member_id"){
					$qryWhereAry[] = "
						order_seller_cancel_off_confirm_member_idx in (Select member_idx From DY_MEMBER Where member_id = N'".trim($_search_paramAryList["search_keyword"])."')
					";
				}else{
	...
	if($_search_paramAryList["date_start"] != "" && $_search_paramAryList["date_end"] != ""){
		if($_search_paramAryList["period_type"] == "cancel_off") {
			$qryWhereAry[] = "
				order_seller_cancel_off_date >= '" . $_search_paramAryList["date_start"] . " 00:00:00' 
				And order_seller_cancel_off_date <= '" . $_search_paramAryList["date_end"] . " 23:59:59'
			";
		}elseif($_search_paramAryList["period_type"] == "order_accept"){
			$qryWhereAry[] = "
				order_progress_step_accept_date >= '" . $_search_paramAryList["date_start"] . " 00:00:00' 
				And order_progress_step_accept_date <= '" . $_search_paramAryList["date_end"] . " 23:59:59'
			";
		}elseif($_search_paramAryList["period_type"] == "order_shipped"){
			$qryWhereAry[] = "
				shipping_date >= '" . $_search_paramAryList["date_start"] . " 00:00:00' 
				And shipping_date <= '" . $_search_paramAryList["date_end"] . " 23:59:59'
			";
		}
	}
...
$args['qry_get_colum'] 	= " 
							...
							, isnull(M.member_id, '') as member_id
";
$args['qry_table_name'] 	= " 
								DY_ORDER O
								Left Outer Join DY_SELLER S On O.seller_idx = S.seller_idx
								Left Outer Join DY_MEMBER M On O.order_seller_cancel_off_confirm_member_idx = M.idx
								Left Outer Join DY_CODE C On C.parent_code = N'ORDER_PROGRESS_STEP' And C.code = O.order_progress_step
";
$args['qry_where'] = " O.order_is_del = N'N' And O.order_is_seller_cancel = N'Y' And O.order_seller_cancel_off = N'Y' ";

...
class BBS extends DBConn
{
	...
	public function insertBBS($args)
	{
		...
		if(!empty($ref_idx)){
			...
			$qry = "Select bbs_step From DY_BBS Where bbs_ref = N'$bbs_ref' And bbs_level = N'".$_ref_view["bbs_level"]."' And bbs_step > N'".$_ref_view["bbs_step"]."' And bbs_is_del = 'N' Order by bbs_step ASC LIMIT 1";
		...
		$qry = "
			Select 
				...
				, Case When name is not null Then name
						...
						When V.vendor_name is not null Then vendor_name
					End as name
				, CM.comment_regdate as regdate
				, Case When CM.member_idx = N'$last_member_idx' Then 1 Else 0 End as is_mine
	...
	public function deleteComment($comment_idx)
	{
		...
		$qry = "
			...
			Set comment_is_del = N'Y', comment_moddate = NOW(), comment_modip = N'$modip', last_member_idx = N'$last_member_idx'

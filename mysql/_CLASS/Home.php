...
class Home extends DBConn
{
	...
	public function getLastSaleStatistics($type) {
        ...
        if ($type == "SalesAmount") {
            $addSelectQry .= ", Sum(settle_sale_sum) as settle_sale_sum";
        } elseif ($type == "SalesCount") {
            $addSelectQry .= ", count(settle_sale_sum) as settle_sale_sum";
        ...
        foreach($_list as $key => $row){
            ...
            $settle_sale_sum = $row["settle_sale_sum"];

            if ($type == "SalesAmount") {
                $settle_sale_sum = round($row["settle_sale_sum"] / 10000);
            }

            $_list[$key]["val"] = $settle_sale_sum;
        }

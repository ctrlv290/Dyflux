...
class Order extends DBConn
{
	...
	public function updateOrderAcceptWholeConfirm($order_accept_temp_list){
		...
		foreach ($_list as $ord)
		{
			...
			$rst = $settle_con->insertSettleFromOrder($order_idx, "order_idx", "CONSIGNMENT");
	...
	public function isUsedInvoiceNo($invoice_no)
	{
		$qry = "
			...
			Where
				...
				And order_is_del = N'N'
			  	AND TIMESTAMPDIFF(DAY, order_regdate, NOW()) < 90
		";
...
//						";
                        ...
                        $qry = "
                                Select
                                product_idx, product_option_idx, stock_unit_price
                                , IFNULL(stock_is_parent_confirm_date, min(stock_is_confirm_date)) as stock_confirm_date
                                , IFNULL(Sum(Case When stock_status = 'NORMAL' Then stock_amount * stock_type Else 0 End), 0) as stock_amount_NORMAL
                                From DY_STOCK
                                Where 1=1
                                    ...
                                    Order by stock_confirm_date ASC , stock_unit_price DESC
                         ";

                        $stock_in_list = parent::execSqlList($qry);
                        ...
                        $stock_out_list = parent::execSqlList($qry);
						...
						if ($stock_in_list) {
							foreach ($stock_in_list as $stock) {
                                ...
                                //가용재고 수량이 없으면 Pass
                                if($stock['stock_amount_NORMAL'] == 0){
                                    continue;
                                }
								...
								if ($order_product_option_cnt > $_currentStockPriceAmount) {
									...
									$_StockOutAry[] = array(
                                        ...
                                        "stock_is_parent_confirm_date" => $stock["stock_confirm_date"]
									);
				...
				if(!$_IsStockOK){
					...
					if(!$isAlreadyStockCheck && $_view["order_progress_step"] == "ORDER_ACCEPT") {
						...
						foreach ($_StockOutAry as $out) {
							$this->insertStockOut($out["product_idx"], $out["product_option_idx"], $out["stock_unit_price"], $out["invoice_cnt"], "INVOICE", "ORDER", "송장입력", $out["order_idx"], $out["order_matching_idx"], $out["stock_is_parent_confirm_date"]);
						}
					}
	 ...
	 * @return int
	 */
	public function insertStockOut($product_idx, $product_option_idx, $stock_unit_price, $stock_amount, $stock_status, $stock_kind, $stock_msg, $order_idx, $order_matching_idx, $stock_is_parent_confirm_date)
	{
		global $GL_Member;
		$modip = $_SERVER["REMOTE_ADDR"];
		$last_member_idx = $GL_Member["member_idx"];
		...
		$qry = "
			Insert Into DY_STOCK
			(
			 stock_ref_idx, product_idx, product_option_idx, stock_kind, order_idx, order_matching_idx, stock_order_idx, stock_is_parent_confirm_date,
		    ...
		    VALUES 
			(
			 0, N'$product_idx', N'$product_option_idx', N'$stock_kind', N'$order_idx', N'$order_matching_idx', 0, N'$stock_is_parent_confirm_date',
	...
	public function updateOrderStepToShippedByInvoiceNo($invoice_no, $cs_msg = ""){
		...
		//order_pack_idx 및 상태값 확인
		$qry = "Select order_pack_idx, order_progress_step From DY_ORDER Where invoice_no = N'$invoice_no' And order_is_del = 'N' LIMIT 1 ";
		$rst = parent::execSqlOneRow($qry);

		if (!$rst || $rst["order_progress_step"] == "ORDER_SHIPPED") {
			$this->sqlTransactionRollback();
			return $return_value;
		}
		...
		";
		$st_rst = parent::execSqlUpdate($qry);
		...
		$qry = "
			Update DY_ORDER
				Set order_progress_step = N'ORDER_SHIPPED'
				    ...
				    , shipping_member_idx = N'$last_member_idx'
					, order_moddate = NOW()
					, order_modip = N'$mod_ip'
					, last_member_idx = N'$last_member_idx'
				Where invoice_no = N'$invoice_no' And order_is_hold = N'N' And order_is_del = N'N'
		";
		$return_value = parent::execSqlUpdate($qry);

		//CS 입력
		$C_CS    = new CS();
		$cs_task = "SHIPPED_CONFIRM";    //배송확인
		$cs_rst  = $C_CS->insertCS($order_pack_idx, $order_pack_idx, 0, 0, 0, "Y", "", $cs_task, $cs_msg, "", "", null, true);

		//정산 테이블 입력
		$settle_con = new Settle();
		$rst = $settle_con->insertSettleFromOrder($order_pack_idx, "order_pack_idx", "SELF");

		if($cs_rst && $rst && $st_rst && $return_value) {
			parent::sqlTransactionCommit();
		} else {
			parent::sqlTransactionRollback();
			$return_value = false;
	...
	public function updateOrderShippedToCancelByInvoiceNo($invoice_no, $cs_msg = "")
	{
		...
		if($order_pack_idx) {
            ...
            $qry = "
			Select OPM.order_matching_idx, P.product_sale_type
			From DY_ORDER O
				Inner Join DY_ORDER_PRODUCT_MATCHING OPM On O.order_idx = OPM.order_idx
				JOIN DY_PRODUCT AS P ON P.product_idx = OPM.product_idx 
			Where 
				O.order_is_del = N'N' 
            ...
            foreach($_opm_list as $ord) {
            	// 위탁은 배송 시, 정산 입력이 아니기 때문에 송장상태로 롤백되어도 정산에 입력하지 않는다.
            	if($ord["product_sale_type"] != "CONSIGNMENT") {
					$order_matching_idx = $ord["order_matching_idx"];
					$C_CS->insertSettleCancel($order_matching_idx,'',true,true);
				}
            }
	 ...
	 * @param $except : 제외주문(이거나 제외 매칭 상품)
	 * @param string $cs_msg : CS 이력 추가 메시지
	 * @return bool
	 */
	...
	public function insertNewAcceptOrder($args)
	{
		...
		//정산 입력 - 위탁
		$settle_con = new Settle();
		$settle_rst = $settle_con->insertSettleFromOrder($order_idx, "order_idx", "CONSIGNMENT");

		if($__order_matching_idx && $settle_rst) {
			$C_CS = new CS();
    ...
    /**
     * 공급처별 주문다운로드 포맷설정 리스트
     * @param $supplier_idx : 공급처 IDX
     * @return array
     */
    public function getOrderDownloadFormatSupplierList($supplier_idx)
    {
        $qry = "
			Select* From DY_ORDER_DOWNLOAD_FORMAT_SUPPLIER 
            Where supplier_idx = N'$supplier_idx' 
            AND order_download_format_is_del =N'N'
            ORDER BY sort ASC;
		";
    ...
    /**
     * 주문다운로드 포맷설정 default 목록
     * @return array
     */
    public function getOrderDownloadFormatList()
    {
        $qry = "
			Select * From dy_order_download_format_default
			Where order_download_format_is_del = N'N'
			order by order_download_format_default_header_name ASC
		";
     ...
     * 주문다운로드 발주서 포맷 저장 함수
     * @param $supplier_idx : 공급처 IDX
     * @param $order_download_format_default_idx : 발주서 기본 헤더 IDX
     * @param $order_download_format_header_name : 발주서 헤더명
     * @param $sort : 정렬 순서
     * @return bool|int|resource
     */
    public function saveOrderDownloadFormat($supplier_idx, $order_download_format_default_idx, $order_download_format_header_name, $sort)
    {
        global $GL_Member;
        $modip = $_SERVER["REMOTE_ADDR"];
        $last_member_idx = $GL_Member["member_idx"];

        $query = "select order_download_format_idx From DY_ORDER_DOWNLOAD_FORMAT_SUPPLIER
                    Where order_download_format_is_del = N'N'
                    AND supplier_idx = N'$supplier_idx'
                    And sort = N'$sort' ";

        parent::db_connect();
        $order_download_format_idx = parent::execSqlOneCol($query);
        parent::db_close();

        if (!$order_download_format_idx) $order_download_format_idx = NULL;
        $qry = "
            Insert Into DY_ORDER_DOWNLOAD_FORMAT_SUPPLIER
            (
                order_download_format_idx, supplier_idx, sort, order_download_format_default_idx, order_download_format_header_name
                , order_download_format_regip, last_member_idx
            ) 
            VALUES 
            (
                N'$order_download_format_idx'
                , N'$supplier_idx'
                , N'$sort'
                , N'$order_download_format_default_idx'
                , N'$order_download_format_header_name'
                , N'$modip'
                , N'$last_member_idx'
            )
            ON DUPLICATE KEY UPDATE
                order_download_format_default_idx = N'$order_download_format_default_idx',
                order_download_format_header_name = N'$order_download_format_header_name',
                order_download_format_moddate = NOW(),
                order_download_format_modip = N'$modip',
                last_member_idx = N'$last_member_idx'
        ";
     ...
     * 주문다운로드 발주서 포맷 삭제 함수
     * @param $supplier_idx : 공급처 IDX
     * param $not_del_sort : 삭제되지 않을 정렬
     * @return bool|int|resource
     */
    public function delOrderDownloadFormat($supplier_idx, $all_del = false, $not_del_sort)
    {
        global $GL_Member;
        $modip = $_SERVER["REMOTE_ADDR"];
        $last_member_idx = $GL_Member["member_idx"];
        $qry = "
                update DY_ORDER_DOWNLOAD_FORMAT_SUPPLIER 
                SET order_download_format_is_del = N'Y',
                    order_download_format_moddate = NOW(),
                    order_download_format_modip = N'$modip',
                    last_member_idx = N'$last_member_idx'
                WHERE supplier_idx = N'$supplier_idx'
        ...
        if(!$all_del){
            $qry .= "AND sort NOT IN ($not_del_sort)";
        }
        parent::db_connect();
        $rst = parent::execSqlUpdate($qry);
        parent::db_close();

...
if($_search_param)
{
	...
	foreach($_search_paramAry as $sitem) {
		...
		if(trim($val) && in_array($col, $avaliable_col)) {
			if($col == "stock_status" || trim($col) == "stock_kind" || trim($col) == "stock_is_confirm") {
				$qryWhereAry[] = $col . " = N'" . $val . "'";
			} elseif(trim($col) == "search_column") {
				if (trim($_search_paramAryList["search_keyword"]) != "") {
					if ($val == "all" || $val == "stock_order_idx") {
						$allWhere = [];
						$allWhere[] = " (
							(A.order_idx != 0 AND A.order_idx = N'" . trim($_search_paramAryList["search_keyword"]) . "')
							Or 
							(A.stock_order_idx != 0 AND A.stock_order_idx = N'" . trim($_search_paramAryList["search_keyword"]) . "')
						)";

						if ($val == "all") {
							$allList = ["product_name", "product_option_name", "O.receive_name"];

							foreach ($allList as $colName) {
								$allWhere[] = $colName . " like N'%" . trim($_search_paramAryList["search_keyword"]) . "%'";
							}
						}

						$allWhereQry = "(" . join(" OR ", $allWhere) . ")";
						$qryWhereAry[] = $allWhereQry;
...
$args['qry_get_colum'] 	= " A.*
							...
							, (Case
								...
								WHEN A.stock_kind = 'SAMPLE' THEN '샘플'
							...
							End)
							as stock_code
							, stock_request_date as stock_request_date_convert
							, (Case
								When A.stock_is_confirm = 'N' Then '미확정'
								When A.stock_is_confirm = 'Y' Then '확정'
							End) as stock_is_confirm_han
							, stock_request_date as stock_request_date2
							, Case When O.order_idx is not null Then
								CONCAT(O.receive_name, '/', O.receive_hp_num, '/', O.receive_addr1, ' ' , O.receive_addr2)
...
$args['qry_table_name'] 	= " DY_STOCK A 
							...
							";
$args['qry_where']			= " A.stock_kind in ('STOCK_ORDER', 'RETURN', 'EXCHANGE', 'BACK', 'SAMPLE')
								And (
									A.stock_order_idx = 0
									Or

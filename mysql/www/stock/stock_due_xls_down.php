...
$qryWhereAry = array();
if($_search_param) {
	foreach($_search_param as $sitem) {
		...
		if(trim($val) && in_array($col, $avaliable_col)) {
			if(
				...
				if(trim($_search_param["search_keyword"]) != "") {
					$qryWhereAry[] = $val . " like N'%" . trim($_search_param["search_keyword"]) . "%'";
				}
			}else{
	...
	$date_search_col = "";
	if($_search_param["date_start"] != "" && $_search_param["date_end"] != ""){
		$qryWhereAry[] = "	 
			stock_regdate >= '".$_search_param["date_start"]." 00:00:00' 
			And stock_regdate <= '".$_search_param["date_end"]." 23:59:59' 
		";
	}

	//공급처
	if($_search_param["supplier_idx"]){
		$qryWhereAry[] = "P.supplier_idx in (" . implode(",", $_search_param["supplier_idx"]) . ")";
	}

...
var SettleTransaction = (function() {
	...
	var TransactionListInit = function(){
		...
		$(".supplier_idx").SumoSelect({
			...
			captionFormatAllSelected : '전체 공급처',
			search: true,
			searchText: '공급처 검색',
			noMatch : '검색결과가 없습니다.'
		});
		...
		$(".seller_idx").SumoSelect({
			...
			placeholder: '전체 판매처',
			captionFormat : '{0}개 선택됨',
			captionFormatAllSelected : '전체 판매처',
			search: true,
			searchText: '판매처 검색',
			noMatch : '검색결과가 없습니다.'
		});
		...
		$('.supplier_idx').on('sumo:opening', function () {
			$('.select-all').css('height', '35px');
			$('.select-all').children('label').text('전체 선택');
		});
	...
	var TransactionListGridInit = function(){
		...
		var _colModel = [
			...
			{ label: '수령자', name: 'receive_name', index: 'receive_name', width: 100, sortable: false, frozen: true},
			...
			{ label: '주소', name: 'receive_addr1', index: 'receive_addr1', width: 100, sortable: false, formatter: function(cellvalue, options, rowobject){
					...
					if(isolateList.includes(rowId)) return 'style="color:red !important;"';
				}},
			...
			{ label: '매출이익', name: 'settle_sale_profit', index: 'settle_sale_profit', width: 120, sortable: false, align: 'right', formatter: 'integer'},
			{ label: '공급가액', name: 'settle_sale_profit_ex_vat', index: 'settle_sale_profit_ex_vat', width: 120, sortable: false, align: 'right', formatter: 'integer'},
		...
		$("#grid_list_transaction").jqGrid({
			...
			loadComplete: function(data){
				...
				if(isDYLogin) {
					...
					$("#grid_list_transaction_settle_sale_profit_ex_vat").addClass("profit").find("div").css({"top": "auto"});
				...
				let profit_sum = userData.profit_sum;

				$(".total_sale_sum").text(Common.addCommas(userData.sale_sum));
				$(".total_purchase_sum").text(Common.addCommas(userData.purchase_sum));
				$(".total_profit_sum").text(Common.addCommas(userData.profit_sum));
			...
			ondblClickRow: function(id){
				if(isDYLogin) {
					let rowData = $('#grid_list_transaction').getRowData(id);
					let settle_closing = rowData.settle_closing;

					if (settle_closing == "N") {

						if (id && id !== lastSelection) {
							let settle_idx = rowData.settle_idx;
							...
							$('#grid_list_transaction').jqGrid('editRow', id, {
								...
								focusField: false,
								oneditfunc: function () {
									let row = $("input[rowid='" + id + "']");
									let taxType = $("#grid_list_transaction").jqGrid("getCell", id, "product_tax_type");
									let taxRatio = 0;
									if(taxType === "과세") taxRatio = 10;

									let optCnt = $("input[rowid='" + id + "'][name='" + "product_option_cnt" + "']");
									let unitPrc = $("input[rowid='" + id + "'][name='" + "order_unit_price" + "']");
									let saleSupPrc = $("input[rowid='" + id + "'][name='" + "settle_sale_supply" + "']");
									let saleSupPrcEx = $("input[rowid='" + id + "'][name='" + "settle_sale_supply_ex_vat" + "']");
									let saleComm = $("input[rowid='" + id + "'][name='" + "settle_sale_commission_in_vat" + "']");
									let saleCommEx = $("input[rowid='" + id + "'][name='" + "settle_sale_commission_ex_vat" + "']");
									let saleDelv = $("input[rowid='" + id + "'][name='" + "settle_delivery_in_vat" + "']");
									let saleDelvEx = $("input[rowid='" + id + "'][name='" + "settle_delivery_ex_vat" + "']");
									let saleDelvComm = $("input[rowid='" + id + "'][name='" + "settle_delivery_commission_in_vat" + "']");
									let saleDelvCommEx = $("input[rowid='" + id + "'][name='" + "settle_delivery_commission_ex_vat" + "']");
									let purUnitPrc = $("input[rowid='" + id + "'][name='" + "settle_purchase_unit_supply" + "']");
									let purUnitPrcEx = $("input[rowid='" + id + "'][name='" + "settle_purchase_unit_supply_ex_vat" + "']");
									let purSupPrc = $("input[rowid='" + id + "'][name='" + "settle_purchase_supply" + "']");
									let purSupPrcEx = $("input[rowid='" + id + "'][name='" + "settle_purchase_supply_ex_vat" + "']");
									let purDelv = $("input[rowid='" + id + "'][name='" + "settle_purchase_delivery_in_vat" + "']");
									let purDelvEx = $("input[rowid='" + id + "'][name='" + "settle_purchase_delivery_ex_vat" + "']");

									// 공급가액
									let exTaxList = [saleSupPrcEx, saleCommEx, saleDelvEx, saleDelvCommEx, purUnitPrcEx, purSupPrcEx, purDelvEx];
									exTaxList.forEach(function(exTax){
										exTax.on("change keyup", function(){
											if($(this).val() !== "") {
												let obj = SettleTransaction.createSettleObject(id);
												SettleTransaction.setSettleSumToTransactionList(obj);
											}
										});
									});

									// 판매수량
									optCnt.on("change keyup", function(){
										if($(this).val() !== "" && unitPrc.val() !== "" && purUnitPrc.val() !== "") {
											saleSupPrc.val($(this).val() * unitPrc.val());
											saleSupPrcEx.val(Math.round(saleSupPrc.val() / ((taxRatio/100) + 1)));

											purSupPrc.val($(this).val() * purUnitPrc.val());
											purSupPrcEx.val(Math.round(purSupPrc.val() / ((taxRatio/100) + 1)));
									...
									// 판매단가
									unitPrc.on("change keyup", function(){
										if(unitPrc.val() !== "" && unitPrc.val() !== "") {
											saleSupPrc.val(optCnt.val() * unitPrc.val());
											saleSupPrcEx.val(Math.round(saleSupPrc.val() / ((taxRatio/100) + 1)));
									...
									// 판매가
									saleSupPrc.on("change keyup", function(){
										if($(this).val() !== "") {
											saleSupPrcEx.val(Math.round($(this).val() / ((taxRatio/100) + 1)));
									...
									// 수수료
									saleComm.on("change keyup", function(){
										if($(this).val() !== "") {
											saleCommEx.val(Math.round($(this).val() / 1.1));
									...
									// 배송비
									saleDelv.on("change keyup", function(){
										if($(this).val() !== "") {
											saleDelvEx.val(Math.round($(this).val() / 1.1));
									...
									// 배송비 수수료
									saleDelvComm.on("change keyup", function(){
										if($(this).val() !== "") {
											saleDelvCommEx.val(Math.round($(this).val() / 1.1));
									...
									// 매입단가
									purUnitPrc.on("change keyup", function(){
										if($(this).val() !== "" && optCnt.val() !== "") {
											purUnitPrcEx.val(Math.round($(this).val() / ((taxRatio/100) + 1)));
											purSupPrc.val($(this).val() * optCnt.val());
											purSupPrcEx.val(Math.round(purSupPrc.val() / ((taxRatio/100) + 1)));
									...
									// 매입가
									purSupPrc.on("change keyup", function(){
										if($(this).val() !== "") {
											purSupPrcEx.val(Math.round($(this).val() / ((taxRatio/100) + 1)));
									...
									// 매입 배송비
									purDelv.on("change keyup", function(){
										if($(this).val() !== "") {
											purDelvEx.val(Math.round($(this).val() / 1.1));
			...
			if(isDYLogin) {
				$("#grid_list_transaction").jqGrid('setGroupHeaders', {
					...
					groupHeaders: [
						...
						{startColumnName: 'settle_sale_profit', numberOfColumns: 2, titleText: '매출이익'},
					]
				});
			}
	...
	let calculateSettle = function(arr){
		let obj = arr;

		obj.settle_sale_sum = obj.settle_sale_supply - obj.settle_sale_commission_in_vat + obj.settle_delivery_in_vat - obj.settle_delivery_commission_in_vat;
		obj.settle_sale_sum_ex_vat = obj.settle_sale_supply_ex_vat - obj.settle_sale_commission_ex_vat + obj.settle_delivery_ex_vat + obj.settle_delivery_commission_ex_vat;

		obj.settle_purchase_sum = obj.settle_purchase_supply + obj.settle_purchase_delivery_in_vat;
		obj.settle_purchase_sum_ex_vat = obj.settle_purchase_supply_ex_vat + obj.settle_purchase_delivery_ex_vat;

		obj.settle_sale_profit = obj.settle_sale_sum - obj.settle_purchase_sum;
		obj.settle_sale_profit_ex_vat = obj.settle_sale_sum_ex_vat - obj.settle_purchase_sum_ex_vat;

		return obj;
	};

	let createSettleObject = function(id){
		let obj = {
			id: id,
			settle_sale_supply: Number($("input[rowid='" + id + "'][name='" + "settle_sale_supply" + "']").val()),
			settle_sale_supply_ex_vat: Number($("input[rowid='" + id + "'][name='" + "settle_sale_supply_ex_vat" + "']").val()),
			settle_sale_commission_in_vat: Number($("input[rowid='" + id + "'][name='" + "settle_sale_commission_in_vat" + "']").val()),
			settle_sale_commission_ex_vat: Number($("input[rowid='" + id + "'][name='" + "settle_sale_commission_ex_vat" + "']").val()),
			settle_delivery_in_vat: Number($("input[rowid='" + id + "'][name='" + "settle_delivery_in_vat" + "']").val()),
			settle_delivery_ex_vat: Number($("input[rowid='" + id + "'][name='" + "settle_delivery_ex_vat" + "']").val()),
			settle_delivery_commission_in_vat: Number($("input[rowid='" + id + "'][name='" + "settle_delivery_commission_in_vat" + "']").val()),
			settle_delivery_commission_ex_vat: Number($("input[rowid='" + id + "'][name='" + "settle_delivery_commission_ex_vat" + "']").val()),
			settle_purchase_unit_supply: Number($("input[rowid='" + id + "'][name='" + "settle_purchase_unit_supply" + "']").val()),
			settle_purchase_unit_supply_ex_vat: Number($("input[rowid='" + id + "'][name='" + "settle_purchase_unit_supply_ex_vat" + "']").val()),
			settle_purchase_supply: Number($("input[rowid='" + id + "'][name='" + "settle_purchase_supply" + "']").val()),
			settle_purchase_supply_ex_vat: Number($("input[rowid='" + id + "'][name='" + "settle_purchase_supply_ex_vat" + "']").val()),
			settle_purchase_delivery_in_vat: Number($("input[rowid='" + id + "'][name='" + "settle_purchase_delivery_in_vat" + "']").val()),
			settle_purchase_delivery_ex_vat: Number($("input[rowid='" + id + "'][name='" + "settle_purchase_delivery_ex_vat" + "']").val())
		};

		return calculateSettle(obj);
	};

	let setSettleSumToTransactionList = function(obj){
		for(let key of Object.entries(obj)){
			$("input[rowid='" + obj.id + "'][name='" + key + "']").val(obj[key]);
		}

		// let rowData = $("#grid_list_transaction").jqGrid("getRowData", obj.id);
		// rowData.sale_sum = obj.settle_sale_sum;
		// rowData.sale_sum_ex_vat = obj.settle_sale_sum_ex_vat;
		// rowData.purchase_sum = obj.settle_purchase_sum;
		// rowData.purchase_sum_ex_vat = obj.settle_purchase_sum_ex_vat;
		// rowData.settle_sale_profit_in_vat = obj.settle_sale_profit_in_vat;
		// rowData.settle_sale_profit = obj.settle_sale_profit;
		// $("#grid_list_transaction").jqGrid("setRowData", obj.id, rowData);
		$("#grid_list_transaction").jqGrid("setCell", obj.id, "sale_sum", obj.settle_sale_sum);
		$("#grid_list_transaction").jqGrid("setCell", obj.id, "sale_sum_ex_vat", obj.settle_sale_sum_ex_vat);
		$("#grid_list_transaction").jqGrid("setCell", obj.id, "purchase_sum", obj.settle_purchase_sum);
		$("#grid_list_transaction").jqGrid("setCell", obj.id, "purchase_sum_ex_vat", obj.settle_purchase_sum_ex_vat);
		$("#grid_list_transaction").jqGrid("setCell", obj.id, "settle_sale_profit", obj.settle_sale_profit);
		$("#grid_list_transaction").jqGrid("setCell", obj.id, "settle_sale_profit_ex_vat", obj.settle_sale_profit_ex_vat);
	};
	...
	return {
		...
		createSettleObject: createSettleObject,
		setSettleSumToTransactionList: setSettleSumToTransactionList
	}
})();
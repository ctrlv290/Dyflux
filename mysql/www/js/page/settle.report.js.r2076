...
var SettleReport = (function() {
	...
	var ReportListSearch = function(){
		...
		ReportListGetData(".CASH_CUSTOMER_IN", "CASH_CUSTOMER_IN", "IN");
		ReportListGetData(".CASH_CUSTOMER_OUT", "CASH_CUSTOMER_OUT", "OUT");
	...
	var ReportAmountCompare = function(){
		...
		if(isNaN(acc_in_amt) || isNaN(bank_in_amt)) return;
		...
		if(isNaN(acc_out_amt) || isNaN(bank_out_amt) ||isNaN(card_out_amt)) return;
	...
	var ReportPopOpen = function(tran_type, tran_inout, mode, tran_idx){
		...
		var wW = 700;
		if(tran_type == "BANK_CUSTOMER_IN" || tran_type == "BANK_CUSTOMER_OUT" || tran_type == "CASH_CUSTOMER_IN" || tran_type == "CASH_CUSTOMER_OUT") wW = 1100;

 ...
 * User: ssawoona
 * Date: 2018
 * Desc: API 사용가능 한 몰들에 대한 주문 취소 처리
...
$C_CS  = new CS();
$C_Order = new Order();
$market_order_no_list = array();
$cs_reason_code1 = "CS_REASON_CANCEL_AUTO";
$cs_reason_code2 = "CANCEL_MARKET_AUTO";
$cs_msg          = "[DY-AUTO] 자동 취소";

switch ($market_code) {
	case "11ST" :
		$C_API_11st  = new API_11st();
		$ret         = $C_API_11st->execCancelProc(array(
	...
	case "COUPANG" :
		...
		$ret           = $C_API_Coupang->execCancelProc(array(
			's_date' => $s_date,
			'e_date' => $e_date,
	...
	case "INTERPARK" :
		...
		$ret           = $C_API_Interpark->execCancelProc(array(
			's_date' => $s_date,
			'e_date' => $e_date,
	...
	case "SSGMALL" :
		...
		$ret           = $C_API_SSGmall->execCancelProc(array(
	...
	case "LOTTECOM" :
		...
		$ret           = $C_API_LotteCom->execCancelProc(array(
			's_date' => str_replace("-", "", $s_date),
			'e_date' => str_replace("-", "", $e_date),
	...
	case "CAFE24" :
		...
		$ret           = $C_API_Cafe24->execCancelProc(array(
			's_date' => $s_date,
			'e_date' => $e_date,
			'seller_idx' => $seller_idx,
			'cs_reason_code1' => $cs_reason_code1,
		...
		break;
	default:
		// 기타 쇼핑몰이면 주문번호로 취소 처리
		if($cancel_orders != "") {
			$ret['status_code']  = 0;
			$ret['result_text']  = "";
			$ret['request_cnt']  = 0;    // 전체 취소 요청 수
			$ret['confirm_cnt']  = 0;    // 취소 숭인 수
			$ret['reject_cnt']   = 0;    // 취소 거부 수 (이미 송장 이상으로)
...
$ret_json['request_cnt']  = $ret['request_cnt'];     // 전체 취소 요청 수
$ret_json['confirm_cnt']  = $ret['confirm_cnt'];     // 취소 숭인 수
$ret_json['reject_cnt']   = $ret['reject_cnt'];      // 취소 거부 수 (이미 송장 이상으로)
$ret_json['notorder_cnt'] = $ret['notorder_cnt'];   // DY 에 주문 내역 없음
$ret_json['deliveryInfo'] = $ret['deliveryInfo'];

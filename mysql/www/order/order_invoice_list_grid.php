		...
		if(trim($val) && in_array($col, $available_col)) {
			if(trim($col) == "order_progress_step") {
				...
				if(trim($_search_paramAryList["search_keyword"]) != "") {
					if ($val == "all") {
						$qryWhereAry[] = "
							(A.order_pack_idx like N'%" . trim($_search_paramAryList["search_keyword"]) . "%'
							OR A.receive_name like N'%" . trim($_search_paramAryList["search_keyword"]) . "%' )
						";
					} else {
						$qryWhereAry[] = "A." . $val . " like N'%" . trim($_search_paramAryList["search_keyword"]) . "%'";
					}
				}
...
$query = $query . "
	...
	, CONCAT(MAX(A.receive_addr1), ' ', MAX(A.receive_addr2)) AS receive_addr

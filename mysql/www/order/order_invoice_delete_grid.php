...
//주문 상태 조건
$qryWhereOrderProgressStepAry = array();
$qryWhereOrderProgressStepAry[] = "'ORDER_INVOICE'";
...
if($_search_param)
{
	...
	if($_search_paramAryList["date"] && $_search_paramAryList["time_start"] && $_search_paramAryList["time_end"]){
		$qryWhereAry[] = " 
				A.invoice_date >= '".$_search_paramAryList["date"]." ".$_search_paramAryList["time_start"]."'
				And A.invoice_date <= '".$_search_paramAryList["date"]." ".$_search_paramAryList["time_end"]."'
		";
	}

	//배송주문 포함 검색 조건

	if($_search_paramAryList["include_shipped"] == "Y"){
		$qryWhereOrderProgressStepAry[] = "'ORDER_SHIPPED'";
	}
}

$qryWhereOrderProgressStepAryJoin = implode(",", $qryWhereOrderProgressStepAry);
$qryWhereAry[] = " A.order_progress_step IN (".$qryWhereOrderProgressStepAryJoin.")";

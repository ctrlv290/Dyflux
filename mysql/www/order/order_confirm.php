			...
			<div class="find_wrap">
				<div class="finder">
					...
					<div class="finder_set">
                        ...
                        <div class="finder_col">
                            <label><input type="checkbox" name="without_hold" value="N" checked="checked">보류 주문 제외</label>
                        </div>
					</div>
				</div>
				<div class="find_btn">
...
<script>
	...
	function orderAcceptCheckedList(checkedList) {
		...
		let p_url = "/order/order_proc.php";
		let dataObj = {};
	...
	$(".btn-accept-cheacked").on("click", function() {
		let selRowId = $("#grid_list").getGridParam('selarrrow');
		...
		let msg = selRowId.length + "건의 주문을 접수 처리하시겠습니까?";
		if(confirm(msg)) {
			let idx_list = [];

			$.each(selRowId, function(i, o){
				let rowData = $("#grid_list").getRowData(o);

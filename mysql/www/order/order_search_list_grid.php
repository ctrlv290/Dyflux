...
if($_search_param)
{
	...
	foreach($_search_paramAry as $sitem) {
		...
		if(trim($val) && in_array($col, $available_col)) {
			...
			}elseif($col == "product_sale_type"){
                if($val == "SELF") {
                    $qryWhereAry[] = " 
                    P.product_sale_type = N'SELF'
				    ";
                }elseif($val == "CONSIGNMENT") {
                    $qryWhereAry[] = " 
                    P.product_sale_type = N'CONSIGNMENT'
				    ";
                }
...
$args['qry_get_colum'] 	= " 
							...
							, P.product_idx, P.product_name, P.product_sale_type
							...
							, (Select code_name From DY_CODE DCC Where DCC.parent_code = N'ORDER_MATCHING_CS' And DCC.code = OPM.order_cs_status) as order_cs_status_han
                            ";
...
$qry = "
	Select 
	IFNULL(Sum(Case When order_progress_step = 'ORDER_SHIPPED' Then 1 Else 0 End), 0) as sum_shipped

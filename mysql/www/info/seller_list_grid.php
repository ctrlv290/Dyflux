	...
	foreach($_search_paramAry as $sitem) {
		...
		if(trim($val) && in_array($col, $avaliable_col)) {
			if($col == "seller_nameidx") {
				$qryWhereAry[] = " (seller_name like N'%" . $val . "%' Or Convert(seller_idx, char) = N'" . $val . "')";
			}else{
...
if($is_view_collect == "true") {
	$args['qry_table_name'] 	= " DY_SELLER A
		...
		SELECT ROW_NUMBER() OVER(PARTITION BY OC.seller_idx ORDER BY OC.order_collect_idx DESC) as order_by_num, OC.*
		FROM DY_ORDER_COLLECT OC WHERE OC.order_collect_is_del = N'N' And OC.collect_type = N'AUTO') TMP 

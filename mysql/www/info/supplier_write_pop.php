			...
			<form name="dyForm" method="post" class="<?php echo $mode?>">
				...
				<div class="tb_wrap">
					<table>
						...
						<tbody>
						<tr>
							<th>공급처 명 <span class="lb_red">필수</span></th>
							<td class="text_left">
								<input type="text" name="supplier_name" class="w400px" maxlength="20" value="<?=$supplier_name?>" />
						...
						<tr>
							...
							<td class="text_left">
								...
								<a href="javascript:;" class="btn orange_btn btn-supplier_group_pop">공급처 그룹 신규 등록</a>
							</td>
						</tr>
						</tbody>
					</table>
					...
					<table>
						...
						<tr>
							<th>거래일</th>
							<td class="text_left">
								거래시작일 : <input type="text" name="supplier_startdate" class="w100px jqDate" readonly="readonly" maxlength="10" value="<?=$supplier_startdate?>" />
								&nbsp;&nbsp;
								거래종료일 : <input type="text" name="supplier_enddate" class="w100px jqDate" readonly="readonly" maxlength="10" value="<?=$supplier_startdate?>" />
							</td>
						</tr>
						<tr>
						...
						<tr>
							<td class="text_left">
								...
								<span class="info_txt col_red">다중입력 가능 (엔터)</span>
                                <a href="javascript:;" id="btn-email-copy" class="btn btn-email-copy">위와 같음</a>
								<div class="row">
                        ...
                        <tr>
                            <th>결제타입  <span class="lb_red">필수</span></th>
                            <td class="text_left">
                                <label><input type="radio" id="supplier_payment_type_m" name="supplier_payment_type" value="MONTH" <?=($supplier_payment_type == "MONTH") ? "checked" : ""?>/> 월 결제 </label>
                                <label><input type="radio" id="supplier_payment_type_d" name="supplier_payment_type" value="DAY" <?=($supplier_payment_type == "DAY") ? "checked" : ""?>/> 일 결제 </label>
                            </td>
                        </tr>
						<tr>
							<th>선급금 사용여부  <span class="lb_red">필수</span></th>
							<td class="text_left">
								<label><input type="radio" id="supplier_use_prepay_y" name="supplier_use_prepay" value="Y" <?=($supplier_use_prepay == "Y") ? "checked" : ""?> /> Y</label>
								<label><input type="radio" id="supplier_use_prepay_n" name="supplier_use_prepay" value="N" <?=($supplier_use_prepay == "N") ? "checked" : ""?>/> N</label>
							</td>
						</tr>
						</tbody>
					</table>
					...
					<table id="add_officer_table1" style="display:none">
						<colgroup>
							<col width="150">
							<col width="*">
						</colgroup>
					...
					<table id="add_officer_table2" style="display:none">
						<colgroup>
							<col width="150">
							<col width="*">
						</colgroup>
					...
					<table id="add_officer_table3" style="display:none">
						<colgroup>
							<col width="150">
							<col width="*">
						</colgroup>
                    ...
                    <div class="btn_set">
                        <div class="center">
                            <a href="javascript:;" id="officer_add_btn" class="btn blue_btn">추가</a>
                            <a href="javascript:;" id="officer_del_btn" class="btn red_btn">삭제</a>
                        </div>
                    </div>
...
<script>
    ...
    var add_officer_count = 1;
    <?php if($mode == "mod"){?>
        if(!Common.isEmpty($('[name="supplier_officer2_name"]').val())){
            $("#add_officer_table1").css("display", "");
            add_officer_count ++;
        }
        if(!Common.isEmpty($('[name="supplier_officer3_name"]').val())){
            $("#add_officer_table2").css("display", "");
            add_officer_count ++;
        }
        if(!Common.isEmpty($('[name="supplier_officer4_name"]').val())) {
            $("#add_officer_table3").css("display", "");
            add_officer_count ++;
        }
    <?php }?>

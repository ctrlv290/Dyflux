...
include "./_xls_default_column_array.php";

$_default_list = $xls_column_ary[$_GET["target"]];
...
<script>
	...
	var default_list = new Array();
	var tmp;
	<?php
	foreach($_default_list as $l){
		$is_readonly = "false";
		if($l["is_readonly"]){
			$is_readonly = "true";
		}
	?>
	tmp = {is_use : true, is_readonly : <?=$is_readonly?>, label : "<?=$l["name"]?>", name : "<?=$l["col"]?>", column_name : "<?=$l["col"]?>"};
	default_list.push(tmp);
	<?php }?>
</script>

<script src="/common/column_load_js_xls.php?target=<?=$_GET["target"]?>"></script>
...
<script>
	...
	ColumnSetting.ColumnSettingXlsListInit();
</script>

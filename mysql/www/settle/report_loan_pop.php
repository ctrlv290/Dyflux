...
$mode = "save_loan_transaction";
$tran_date = $_GET["tran_date"];
$C_Loan = new Loan();
$_bank_list = $C_Loan->getTodayLoanTransaction($tran_date);
					...
					<table>
						...
						foreach($_bank_list as $bank) {
							...
							<tr>
								<td class="text_left">
									<?=$bank["loan_name"]?>
									<input type="hidden" name="loan_idx[]" value="<?=$bank["loan_idx"]?>" />
								</td>
								<td>
									<input type="text" name="tran_in[]" class="w100per money" value="<?=$tran_in != '0' ? $tran_in : ''?>" />
								</td>
								<td>
...
<script src="/js/page/settle.report.js?v=200424"></script>
<script>
	SettleReport.ReportLoanPopInit();
</script>
